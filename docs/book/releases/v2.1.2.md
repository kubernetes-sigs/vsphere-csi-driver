...
# vSphere CSI Driver - v2.1.2 release

## New Feature

- There is no new feature released in v2.1.2.  v2.1.2 is the patch release to fix an issue observed in [v2.1.1](v2.1.1.md) release.
...
- https://github.com/kubernetes-sigs/vsphere-csi-driver/tree/v2.1.2/manifests/v2.1.2
...
9. `user`, `ca-file` and `VirtualCenter` change in the [vsphere-config-secret](https://vsphere-csi-driver.sigs.k8s.io/driver-deployment/installation.html#create-a-configuration-file-with-vsphere-credentials-) is not honored until vSphere CSI Controller Pod restarts.
...
10. vCenter restart may leave Kubernetes persistent volume claims in the Pending state.
    - Impact:
