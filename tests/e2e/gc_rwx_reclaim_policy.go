...
var _ = ginkgo.Describe("[rwm-csi-tkg] File Volume Test for Reclaim Policy", func() {
	...
	*/
	ginkgo.It("Verify Reclaim Policy Retain with file volume", func() {
		...
		gomega.Expect(queryResult.Volumes[0].HealthStatus).Should(gomega.BeEquivalentTo(healthGreen))
		...
		persistentvolumes[0], err = client.CoreV1().PersistentVolumes().Update(ctx, persistentvolumes[0],
		...
		err = verifyVolumeMetadataInCNS(&e2eVSphere, volumeID, pvclaim.Name, persistentvolumes[0].Name, pod.Name)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("Verify ReadWriteOnce PVC is not usable as ReadWriteMany", func() {
		...
		// Delete POD
		ginkgo.By(fmt.Sprintf("Deleting the pod %s in namespace %s", pod.Name, pod.Namespace))
		err = fpod.DeletePodWithWait(client, pod)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		ginkgo.By("Deleting the PVClaim 2")
		framework.ExpectNoError(fpv.DeletePersistentVolumeClaim(client, pvc2.Name, pvc2.Namespace),
			"Failed to delete PVC", pvc2.Name)

		ginkgo.By("Deleting the PV2")
		framework.ExpectNoError(fpv.DeletePersistentVolume(client, pv2.Name))

		framework.ExpectNoError(fpv.WaitForPersistentVolumeDeleted(client, pv2.Name, poll, pollTimeout))
	...
	*/
	ginkgo.It("Verify ReadWriteMany PVC is usable by another PVC as static provision", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		ginkgo.By("Deleting the PVClaim")
