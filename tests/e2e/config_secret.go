...
var _ = ginkgo.Describe("Config-Secret", func() {
	...
	*/
	...
	ginkgo.It("[csi-config-secret-block][csi-config-secret-file] Change vcenter users in vsphere config "+
		...
		err = createTestUser(vcAddress, sshClientConfig, masterIp, configSecretTestUser1,
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = setHostLevelPermission(vcAddress, sshClientConfig, masterIp, configSecretUser2Alias, hosts)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = setVMLevelPermission(vcAddress, sshClientConfig, masterIp, configSecretUser2Alias,
		...
		for i := 0; i < len(dataCenters); i++ {
			err = setDataStoreLevelPermission(vcAddress, sshClientConfig, masterIp,
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = setSearchlevelPermission(vcAddress, sshClientConfig, masterIp, configSecretUser2Alias,
	...
	*/
	...
	ginkgo.It("[csi-config-secret-block][csi-config-secret-file] VC with Custom Port", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		ginkgo.By("Create testuser1")
		err = createTestUser(vcAddress, sshClientConfig, masterIp, configSecretTestUser1,
		...
		err = createCsiVsphereSecret(secretConf.Global.ClusterID,
			secretConf.Global.ClusterDistribution, secretConf.Global.VCenterHostname,
			secretConf.Global.InsecureFlag,
			configSecretUser1Alias, configSecretTestUser1Password, vCenterNonDefaultPort,
			secretConf.Global.Datacenters, sshClientConfig, masterIp)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			ginkgo.By("Reverting back csi-vsphere.conf with its original vcenter user " +
	...
	*/
	...
	ginkgo.It("[csi-config-secret-block][csi-config-secret-file] Modify VC Port and validate the workloads", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		ginkgo.By("Create testuser1")
		err = createTestUser(vcAddress, sshClientConfig, masterIp, configSecretTestUser1,
		...
		err = createCsiVsphereSecret(secretConf.Global.ClusterID,
			secretConf.Global.ClusterDistribution, secretConf.Global.VCenterHostname,
			secretConf.Global.InsecureFlag,
			configSecretUser2Alias, configSecretTestUser1Password, dummyvCenterPort,
			secretConf.Global.Datacenters, sshClientConfig, masterIp)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
