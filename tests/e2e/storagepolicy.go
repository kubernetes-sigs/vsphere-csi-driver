...
var _ = ginkgo.Describe("Storage Policy Based Volume Provisioning", func() {
	...
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] [csi-supervisor] [csi-guest] Verify dynamic "+
		...
		verifyStoragePolicyBasedVolumeProvisioning(f, client, adminClient,
	...
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] [csi-supervisor] [csi-guest] Verify dynamic "+
		...
		if vanillaCluster {
			...
			ginkgo.By("CNS_TEST: Running for WCP setup")
			_, err := adminClient.StorageV1().StorageClasses().Get(ctx,
		...
		pvc := invokeInvalidPolicyTestNeg(client, adminClient, namespace, scParameters,
	...
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] Verify non-existing SPBM policy is not honored "+
		...
		pvc := invokeInvalidPolicyTestNeg(client, adminClient, namespace, scParameters, scParamStoragePolicyName, pollTimeoutShort)
		if guestCluster {
...
func verifyStoragePolicyBasedVolumeProvisioning(f *framework.Framework, client clientset.Interface,
	adminClient clientset.Interface, namespace string, scParameters map[string]string, storagePolicyName string) {
	...
	pv := getPvFromClaim(client, adminClient, pvclaim.Namespace, pvclaim.Name)
...
func invokeInvalidPolicyTestNeg(client clientset.Interface, adminClient clientset.Interface, namespace string, scParameters map[string]string,
