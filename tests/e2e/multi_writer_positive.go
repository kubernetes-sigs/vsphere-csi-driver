...
var _ bool = ginkgo.Describe("[multi-writer-positive] Multi-Writer-Positive", func() {
	...
	*/
	...
	ginkgo.It("TC1", ginkgo.Label(p0, vmServiceVm, vc901, newTest), func() {
		...
		namespace, statuscode, err := createtWcpNsWithZonesAndPolicies(vcRestSessionId,
			[]string{sharedVmfsStorageProfileId}, getSvcId(vcRestSessionId),
		...
		pvclaimList, err := createPVCsOnScaleWithDifferentConfigurations(ctx, client, namespace, pvclaimlabels,
			diskSize, storageclass, accessMode, rawBlockVolumeMode,
			cfg.CreatePvcCountLimit, cfg.ParallelVolumeCreation, cfg.LateBinding)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		_, vms, _, err := createVmServiceOnScaleWithDifferentSettings(ctx, client, vmopC, cnsopC, namespace,
			pvclaimList, vmClass, storageclass.Name, cfg.VmCreationCount, cfg.ParallelVmCreation,
	...
	/*
		...
		Take a snapshot of PVC created in step #2
		Wait for Snapshot to get created.
		Trigger 2 VMs creation in parallel with configurations  mentioned in the testcase specification and attach it to PVC created in step1
		Wait for VMs to get an IP and to be in power on state.
		Verify volume attached successfully for vm1-pvc and vm2-pvc
		Verify CR cnsnodevmattachment for the PVC and VMs. Should display 2 entries.
		Write some IO to the volume from VMs.
	...
	*/
	...
	ginkgo.It("TC2", ginkgo.Label(p0, vmServiceVm, vc901, newTest), func() {
		...
		ginkgo.By("Create a WCP namespace and add multi-writer capable shared storage policy to ns")
		namespace, statuscode, err = createtWcpNsWithZonesAndPolicies(vcRestSessionId,
			[]string{multiWriterCapableStorageProfileId}, getSvcId(vcRestSessionId),
			[]string{}, vmClass, contentLibId)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		gomega.Expect(statuscode).To(gomega.Equal(status_code_success))
		defer func() {
		...
		ginkgo.By("Deploy WSFC configured 7 VMs")
		_, _, _, err = createVmServiceOnScaleWithDifferentSettings(ctx, client, vmopC, cnsopC, namespace,
			pvclaimList, vmClass, storageclass.Name, vmCreationCount, parallelVmCreation,
			sharingMode, diskMode, busSharing,
			controllerType, controllerNumber, unitNumber)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		// need to fix this code for cns nodeattachment CR
