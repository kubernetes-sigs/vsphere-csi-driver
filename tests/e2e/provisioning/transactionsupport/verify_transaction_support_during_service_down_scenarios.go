...
func createVolumeWithServiceDown(serviceNames []string, namespace string, client clientset.Interface,
	...
	for i := range volumeOpsScale {
		...
		go createPVC(ctx, client, namespace, constants.DiskSize10GB, storageclass, accessMode, pvclaims, i, &wg)
	}
	...
	gomega.Expect(len(pvclaims) == volumeOpsScale).NotTo(gomega.BeFalse()) //TODO

	newdiskSizeInMb := constants.DiskSizeInMinMb * int64(volumeOpsScale)
	...
	isMatched, deltaUsedSpace := k8testutil.ValidateSpaceUsageAfterResourceCreationUsingDatastoreFcdFootprint(dsFcdFootprintMapBeforeProvisioning, dsFcdFootprintMapAfterProvisioning, newdiskSizeInBytes, volumeOpsScale)
	framework.Logf("Is datastore used space matched : %t, Delta Used Space If any : %d", isMatched, deltaUsedSpace)
	gomega.Expect(isMatched).NotTo(gomega.BeFalse())

	// pvcUsability(ctx, e2eTestConfig, client, namespace, storageclass, pvclaims)
}
