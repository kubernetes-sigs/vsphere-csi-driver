...
func createVolumeWithServiceDown(serviceNames []string, namespace string, client clientset.Interface,
	...
	usedSpaceRetVal, numberOfVmdksRetVal, numberOfFcdsRetVal, numberOfVolumesRetVal, deltaUsedSpace := k8testutil.ValidateSpaceUsageAfterResourceCreationUsingDatastoreFcdFootprint(dsFcdFootprintMapBeforeProvisioning, dsFcdFootprintMapAfterProvisioning, newdiskSizeInBytes, volumeOpsScale)
	...
	gomega.Expect(numberOfVmdksRetVal).NotTo(gomega.BeFalse(), "Used space not matched")
	gomega.Expect(numberOfFcdsRetVal).NotTo(gomega.BeFalse(), "Used space not matched")
	gomega.Expect(numberOfVolumesRetVal).NotTo(gomega.BeFalse(), "Used space not matched")
