...
var _ = ginkgo.Describe("Transaction_Support_Register_Volume_Static", func() {
		...
		ginkgo.DescribeTableSubtree("Transaction_Support_Register_Volume_Table_Tests",
			func(serviceNames []string) {
				...
				ginkgo.It("[csi-block-vanilla] [csi-guest] [csi-supervisor] "+
					...
					staticProvisioningRegisterVolumeWithServiceDown(serviceNames, namespace, client, storagePolicyName, scParameters, volumeOpsScale, c)
				})
			},
			entries,
		)
...
// staticProvisioningRegisterVolumeWithServiceDown register fcd and immediately restart the services and wait for
...
func staticProvisioningRegisterVolumeWithServiceDown(serviceNames []string, namespace string, client clientset.Interface, storagePolicyName string, scParameters map[string]string, volumeOpsScale int, c clientset.Interface) {
	...
	fcdIDs := make([]string, volumeOpsScale)
	...
	defer func() {
		if deleteFCDRequired {
			...
			for _, fcdID := range fcdIDs {
				framework.Logf("Deleting FCD : %s", fcdID)
				err := vcutil.DeleteFCD(ctx, fcdID, e2eTestConfig, dsRef)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}
	}()
	...
	framework.Logf("CreateVolume-------------------------%v", serviceNames)
