...
var _ = ginkgo.Describe("Transaction_Support_CreateVolumeFromSnapshot", func() {
	...
	ginkgo.Context("When one or more services are down", func() {
		...
		ginkgo.DescribeTableSubtree("Transaction_Support_CreateVolumeFromSnapshot_Table_Tests",
			func(serviceNames []string) {
				...
				ginkgo.AfterEach(func() {
					...
					//Deleting pvcs created from snapshot
				...
				ginkgo.It("[csi-block-vanilla] [csi-guest] [csi-supervisor] "+
					"Veify Create Volume From Snapshot With Transaction Support During Service Down-APD-vSAN-Partitioning", ginkgo.Label(constants.P0, constants.Disruptive, constants.Block,
					...
					createVolumeFromSnapshotWithServiceDown(serviceNames, namespace, client, storagePolicyName,
...
func createVolumeFromSnapshotWithServiceDown(serviceNames []string, namespace string, client clientset.Interface,
	...
	framework.Logf("CreateVolumeFromSnapshot-------------------------%v", serviceNames)
