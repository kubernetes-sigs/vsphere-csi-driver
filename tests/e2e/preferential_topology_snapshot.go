...
var _ = ginkgo.Describe("[Preferential-Topology-Snapshot] Preferential Topology Volume Snapshot tests", func() {
	...
	*/
	...
	ginkgo.It("Create restore snapshot of pvc using single datastore preference", ginkgo.Label(p0,
		...
		pod, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim2}, admissionapi.LevelBaseline, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
	...
	*/
	...
	ginkgo.It("Create restore snapshot of pvc when multiple preferred datastores are tagged "+
		...
		pod3, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim3}, admissionapi.LevelBaseline, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		pod4, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim4}, admissionapi.LevelBaseline, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		ginkgo.By("Creating Pod")
		pod5, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim5}, admissionapi.LevelBaseline, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
