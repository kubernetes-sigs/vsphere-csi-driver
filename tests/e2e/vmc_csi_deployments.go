...
var _ = ginkgo.Describe("[vmc-gc] [csi-guest] Deploy Update and Scale Deployments", func() {
	...
	*/
	ginkgo.It("Deployments with volume provisioning", func() {
		...
		deployment, err := createDeployment(ctx, client, replica, lables, nil, namespace, pvclaims, "", false, "k8s.gcr.io/nginx-slim:0.8")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		deployment, err = client.AppsV1().Deployments(namespace).Get(ctx, deployment.Name, metav1.GetOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		ginkgo.By("Updating the Deployment with max surge and max unaavailable values")
		...
		updateDeploymentImage(namespace, "k8s.gcr.io/nginx-slim:0.9", deployment.Name, "write-pod")

		ginkgo.By("Scale down replicas")
		updateDeploymentReplica(client, maxReplica, deployment.Name, namespace)
