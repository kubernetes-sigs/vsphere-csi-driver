...
/*
This util will create intial namespace get/post api call request
*/
...
func createtWcpNsWithZonesAndPolicies(
	...
	supervisorId string, zoneNames []string) (string, error) {
	...
	if statusCode != 204 {
		return "", fmt.Errorf("failed to create namespace %v: received status code %d", namespace, statusCode)
	}
