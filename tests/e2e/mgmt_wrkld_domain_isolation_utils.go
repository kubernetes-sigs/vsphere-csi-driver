...
// Assuming `invokeVCRestAPIGetRequest` returns a status code
func markZoneForRemovalFromWcpNs(vcRestSessionId string, namespace string, zone string) error {
	...
	_, statusCode := invokeVCRestAPIGetRequest(vcRestSessionId, deleteZoneFromNs)
	gomega.Expect(statusCode).Should(gomega.BeNumerically("==", 204))
	if statusCode != 204 {
...
func PvcCreationWithTopology(namespace string, ds string, storageclass *storagev1.StorageClass,
