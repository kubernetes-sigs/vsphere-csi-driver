...
func createVolumeWithServiceDown(serviceName string, namespace string, client clientset.Interface,
	...
	if serviceName == "CSI" {
		...
		isServiceStopped, err = stopKubeSystemPods(ctx, c, kubeSystemNamespace)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		defer func() {
			if isServiceStopped {
				framework.Logf("Starting storage-quota-webhook driver")
				isServiceStopped, err = startKubeSystemPods(ctx, c, csiReplicaCount, kubeSystemNamespace)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}()
	...
	if !vanillaCluster {
		...
		quotavalidationStatus := validate_totalStoragequota(ctx, diskSizeInMb*int64(volumeOpsScale), totalquota_used_before, totalquota_used_after)
		gomega.Expect(quotavalidationStatus).NotTo(gomega.BeFalse())
		quotavalidationStatus = validate_totalStoragequota(ctx, diskSizeInMb*int64(volumeOpsScale), storagepolicyquota_pvc_before, storagepolicyquota_pvc_after)
		gomega.Expect(quotavalidationStatus).NotTo(gomega.BeFalse())
		quotavalidationStatus = validate_totalStoragequota(ctx, diskSizeInMb*int64(volumeOpsScale), storagepolicy_usage_pvc_before, storagepolicy_usage_pvc_after)
		gomega.Expect(quotavalidationStatus).NotTo(gomega.BeFalse())
...
func extendVolumeWithServiceDown(serviceName string, namespace string, client clientset.Interface,
	...
	if !vanillaCluster {
		totalquota_used_after_expansion, _ := getTotalQuotaConsumedByStoragePolicy(ctx, restConfig, storageclass.Name, namespace)
		framework.Logf("totalquota_used_after_expansion :%v", totalquota_used_after_expansion)

		storagepolicyquota_pvc_after_expansion, _ := getStoragePolicyQuotaForSpecificResourceType(ctx, restConfig,
			storageclass.Name, namespace, volExtensionName)
		framework.Logf("********** storagepolicyquota_pvc_after_expansion :%v", storagepolicyquota_pvc_after_expansion)

		storagepolicy_usage_pvc_after_expansion, _ := getStoragePolicyUsageForSpecificResourceType(ctx, restConfig,
			storageclass.Name, namespace, pvcUsage)
		framework.Logf("********** storagepolicy_usage_pvc_after_expansion :%v", storagepolicy_usage_pvc_after_expansion)

		//New size is 6Gi, diskSizeInMb is 2Gi so multiplying by 3 to make the expected quota consumption value
		quotavalidationStatus := validate_totalStoragequota(ctx, diskSizeInMb*3*int64(volumeOpsScale), totalquota_used_before, totalquota_used_after_expansion)
		gomega.Expect(quotavalidationStatus).NotTo(gomega.BeFalse())
		quotavalidationStatus = validate_totalStoragequota(ctx, diskSizeInMb*3*int64(volumeOpsScale), storagepolicyquota_pvc_before, storagepolicyquota_pvc_after_expansion)
		gomega.Expect(quotavalidationStatus).NotTo(gomega.BeFalse())
		quotavalidationStatus = validate_totalStoragequota(ctx, diskSizeInMb*3*int64(volumeOpsScale), storagepolicy_usage_pvc_before, storagepolicy_usage_pvc_after_expansion)
		gomega.Expect(quotavalidationStatus).NotTo(gomega.BeFalse())
	}
}
