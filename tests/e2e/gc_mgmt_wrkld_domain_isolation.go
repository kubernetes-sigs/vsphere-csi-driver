...
var _ bool = ginkgo.Describe("[domain-isolation] Management-Workload-Domain-Isolation", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		gcNamespace = GetAndExpectStringEnvVar(envSupervisorClusterNamespace)
	...
	*/
	...
	ginkgo.It("Dynamic and Pre-Provisioned snapshot creation with removal of zones from the namespace", func() {
		...
		err = markZoneForRemovalFromWcpNs(vcRestSessionId, gcNamespace,
			zone2)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		scaleUpAndVerifySts(ctx, client, statefulset, replicas, namespace)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		statusCode := markZoneForRemovalFromNs(gcNamespace, zone3, vcRestSessionId)
		...
		// Create a static snapshot of any 1 snapshot created above.
