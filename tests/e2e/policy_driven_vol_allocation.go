...
var _ = ginkgo.Describe("[vol-allocation] Policy driven volume space allocation tests", func() {
	...
	ginkgo.It("[csi-block-vanilla][csi-guest][csi-supervisor] Verify online LZT/EZT volume expansion of "+
		...
		defer func() {
			...
			for i, pvc := range pvcs {
				err := fpv.DeletePersistentVolumeClaim(ctx, client, pvc.Name, namespace)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, pvs[i].Spec.CSI.VolumeHandle)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}()
	...
	ginkgo.It("[csi-supervisor][csi-block-vanilla][csi-block-vanilla-parallelized][csi-guest]"+
		...
		defer func() {
			...
			for i, pvc := range pvcs {
				err := fpv.DeletePersistentVolumeClaim(ctx, client, pvc.Name, namespace)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, volIds[i])
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}()
	...
	*/
	ginkgo.It("[csi-guest][csi-supervisor][csi-block-vanilla] Verify EZT offline volume expansion", func() {
		...
		defer func() {
			...
			err = fpod.DeletePodWithWait(ctx, client, newPods[0])
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}()
	...
	*/
	ginkgo.It("[csi-block-vanilla] verify volume allocation change post snapshot deletion works fine", func() {
		...
		for i, pvc := range pvcs {
			...
			err = waitForCNSSnapshotToBeCreated(ctx, volIds[i], snapshotId)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}
		defer func() {
			if len(snaps) > 0 {
				...
				for i, snapshotId := range snapIDs {
					...
					err = waitForCNSSnapshotToBeDeleted(ctx, volIds[i], snapshotId)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
				}
			}
		}()
	...
	ginkgo.It("[csi-block-vanilla][csi-guest][csi-supervisor] Verify expansion during Thin -> EZT, LZT -> EZT"+
		...
		defer func() {
			...
			for _, volId := range volIds {
				err = e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, volId)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}()
	...
	*/
	ginkgo.It("[csi-block-vanilla][csi-block-vanilla-parallelized]"+
		...
		ginkgo.By("Delete the pod created")
		err = fpod.DeletePodWithWait(ctx, client, pods[0])
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("[csi-block-vanilla][csi-block-vanilla-parallelized]"+
		...
		for _, snap := range snaps {
			...
			defer func() {
				...
				err = waitForCNSSnapshotToBeDeleted(ctx, snapToVolIdMap[volumeSnapshot.Name], snapshotId)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}()

			ginkgo.By("Query CNS and check the volume snapshot entry")
			err = waitForCNSSnapshotToBeCreated(ctx, snapToVolIdMap[volumeSnapshot.Name], snapshotId)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}
