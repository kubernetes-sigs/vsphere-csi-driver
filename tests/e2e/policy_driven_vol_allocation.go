...
func fillVolumeInPods(f *framework.Framework, client clientset.Interface, pods []*v1.Pod) {
	for _, pod := range pods {
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		writeRandomDataOnPod(pod, originalFsSize-100) // leaving 100m for FS metadata
	}
}
...
func writeRandomDataOnPod(pod *v1.Pod, count int64) {
	var cmd []string
	if windowsEnv {
		cmd = []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "$out = New-Object byte[] 536870912; (New-Object Random).NextBytes($out); [System.IO.File]::WriteAllBytes('/mnt/volume1/testdata2.txt', $out)"}
	} else {
...
func writeKnownData2PodInParallel(
	f *framework.Framework, client clientset.Interface, pod *v1.Pod, testdataFile string, wg *sync.WaitGroup, size ...int64) {
...
func writeKnownData2Pod(f *framework.Framework, client clientset.Interface, pod *v1.Pod, testdataFile string, size ...int64) {
	...
	if wcpVsanDirectCluster {
		...
		if windowsEnv {
			cmdTestData := []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "$out = New-Object byte[] 104857600; (New-Object Random).NextBytes($out); [System.IO.File]::WriteAllBytes('/mnt/volume1/testdata2.txt', $out)"}
	...
	for i := int64(0); i < iosize; i = i + 100 {
		...
		if wcpVsanDirectCluster {
			...
			var cmd []string
			if windowsEnv {
				// cmd = []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "$inputFile = '/mnt/volume1/testdata2.txt'; $outputFile = '/mnt/volume1/testdata3.txt'; $blockSize = 1MB; $count = 100; $seek = seek; $fs = [System.IO.File]::Open($outputFile, 'OpenOrCreate', 'Write'); $fs.Seek($seek * $blockSize, [System.IO.SeekOrigin]::Begin) | Out-Null; [byte[]]$buffer = New-Object byte[] $blockSize; $input = [System.IO.File]::Open($inputFile, 'Open', 'Read'); 1..$count | ForEach-Object { $bytesRead = $input.Read($buffer, 0, $blockSize); if ($bytesRead -le 0) { break } $fs.Write($buffer, 0, $bytesRead) }; $input.Close(); $fs.Close()"}
				cmd = []string{
					"exec",
					pod.Name,
	...
	if wcpVsanDirectCluster {
		...
		if windowsEnv {
			cmd = []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "Remove-Item -Path '/mnt/volume1/testdata2.txt' -Force"}
		} else {
			cmd = []string{"--namespace=" + pod.Namespace, "-c", pod.Spec.Containers[0].Name, "exec", pod.Name, "--",
...
func verifyKnownDataInPod(f *framework.Framework, client clientset.Interface, pod *v1.Pod, testdataFile string, size ...int64) {
	var svcMasterIp string
	var sshWcpConfig *ssh.ClientConfig
	if wcpVsanDirectCluster {
		svcMasterIp = GetAndExpectStringEnvVar(svcMasterIP)
		...
		if windowsEnv {
			cmdTestData := []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "Copy-Item -Path '/mnt/volume1/testdata2.txt' -Destination '/mnt/volume1/testdata2_pod.txt'"}
			_ = e2ekubectl.RunKubectlOrDie(pod.Namespace, cmdTestData...)
		} else {
			_ = e2ekubectl.RunKubectlOrDie(pod.Namespace, "cp", testdataFile, fmt.Sprintf(
				"%v/%v:mnt/volume1/testdata", pod.Namespace, pod.Name))
	...
	for i := int64(0); i < iosize; i = i + 100 {
		...
		if wcpVsanDirectCluster {
			...
			if windowsEnv {
				cmd = []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "$inputFile = '/mnt/volume1/testdata3.txt'; $outputFile = '/mnt/volume1/testdata2.txt'; $blockSize = 1MB; $count = 100; $seek = 10; $fs = [System.IO.File]::Open($outputFile, 'OpenOrCreate', 'Write'); $fs.Seek($seek * $blockSize, [System.IO.SeekOrigin]::Begin) | Out-Null; [byte[]]$buffer = New-Object byte[] $blockSize; $input = [System.IO.File]::Open($inputFile, 'Open', 'Read'); 1..$count | ForEach-Object { $bytesRead = $input.Read($buffer, 0, $blockSize); if ($bytesRead -le 0) { break } $fs.Write($buffer, 0, $bytesRead) }; $input.Close(); $fs.Close()"}
			} else {
				cmd = []string{"--namespace=" + pod.Namespace, "-c", pod.Spec.Containers[0].Name, "exec", pod.Name, "--",
		...
		if windowsEnv {
			cmdTestData := []string{"exec", pod.Name, "--namespace=" + pod.Namespace, "powershell.exe", "((Get-FileHash '/mnt/volume1/testdata2.txt' -Algorithm SHA256).Hash -eq (Get-FileHash '/mnt/volume1/testdata2_pod.txt' -Algorithm SHA256).Hash)"}
