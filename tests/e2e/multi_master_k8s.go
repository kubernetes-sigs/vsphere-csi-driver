...
var _ = ginkgo.Describe("[csi-multi-master-block-e2e]", func() {
	...
	*/
	ginkgo.It("[pq-vanilla-block][pq-n1-vanilla-block][pq-n2-vanilla-block]Power off the node where"+
	...
	*/
	...
	ginkgo.It("[csi-block-vanilla][csi-supervisor][pq-vanilla-block][pq-n1-vanilla-block][pq-n2-vanilla-block]"+
		"Stop kubelet on the node where "+
		"vsphere-csi-controller pod is running", ginkgo.Label(p0, block, vanilla, vc70), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		nodeList, podList := getControllerRuntimeDetails(client, controllerNamespace)
