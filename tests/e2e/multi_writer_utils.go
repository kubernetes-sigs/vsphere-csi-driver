...
func createPVCsOnScaleWithDifferentConfigurations(ctx context.Context,
	...
	createPvcCountLimit int, parallelVolumeCreation bool,
	lateBinding bool) ([]*v1.PersistentVolumeClaim, error) {

	ginkgo.By(fmt.Sprintf(
		"Namespace: %q, DiskSize: %q, AccessMode: %q, VolumeMode: %q, Labels: %+v, StorageClass: %+v, PVC Count: %d, LateBinding: %t",
		namespace, diskSize, accessMode, volumeMode, pvclaimlabels, storageclass, createPvcCountLimit, lateBinding,
	))

	// Modify storage class for LateBinding if requested
	if lateBinding && storageclass != nil {
		waitMode := storagev1.VolumeBindingWaitForFirstConsumer
		storageclass.VolumeBindingMode = &waitMode
		framework.Logf("LateBinding enabled: Setting StorageClass %s to WaitForFirstConsumer", storageclass.Name)
	}
	...
	if parallelVolumeCreation {
		...
		for i := 0; i < createPvcCountLimit; i++ {
			...
			go func(i int) {
				...
				ginkgo.By(fmt.Sprintf(
					"Creating PVC [%d/%d] (parallel) with StorageClass: %s, DiskSize: %q, LateBinding: %t",
					i+1, createPvcCountLimit, storageclass.Name, diskSize, lateBinding,
				))
				...
				if lateBinding {
					// Ensure PVC stays Pending until bound
					err = fpv.WaitForPersistentVolumeClaimPhase(v1.ClaimPending, client, namespace, pvc.Name, framework.Poll, time.Minute)
					if err != nil {
						errCh <- fmt.Errorf("PVC [%d/%d] did not remain Pending: %v", i+1, createPvcCountLimit, err)
						return
					}
					framework.Logf("PVC %s is Pending (LateBinding)", pvc.Name)
				}
		...
		for i := 0; i < createPvcCountLimit; i++ {
			...
			ginkgo.By(fmt.Sprintf(
				"Creating PVC [%d/%d] (serial) with StorageClass: %s, DiskSize: %q, LateBinding: %t",
				i+1, createPvcCountLimit, storageclass.Name, diskSize, lateBinding,
			))
			...
			if lateBinding {
				err = fpv.WaitForPersistentVolumeClaimPhase(v1.ClaimPending, client, namespace, pvc.Name, framework.Poll, time.Minute)
				if err != nil {
					return nil, fmt.Errorf("PVC [%d/%d] did not remain Pending: %v", i+1, createPvcCountLimit, err)
				}
				framework.Logf("PVC %s is Pending (LateBinding)", pvc.Name)
			}
