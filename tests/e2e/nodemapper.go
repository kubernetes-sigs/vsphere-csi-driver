...
func (nm *NodeMapper) GenerateNodeMap(vs *vSphere, nodeList v1.NodeList) error {
	...
	if vs.Config.Global.Datacenters == "" {
		datacenters, err = vs.getAllDatacenters(ctx)
		if err != nil {
			framework.ExpectNoError(err, "NodeMapper error, ")
		}
	} else {
		dcName := strings.Split(vs.Config.Global.Datacenters, ",")
	...
	for _, dc := range datacenters {
		framework.Logf("Search candidates vc=%s and datacenter=%s", vs.Config.Global.VCenterHostname, dc.Name())
	...
	for _, node := range nodeList.Items {
		...
		go func() {
			...
			for _, res := range queueChannel {
				ctx, cancel := context.WithCancel(context.Background())
				defer cancel()
				vm, err := res.vs.getVMByUUID(ctx, nodeUUID)
				if err != nil {
					framework.Logf("Error %v while looking for node=%s in vc=%s",
						err, n.Name, res.vs.Config.Global.VCenterHostname)
					continue
				}
				if vm != nil {
...
// withTagsClient establish rest connection to retrieve tag manager stub
func withTagsClient(ctx context.Context, connection *vSphere, f func(c *rest.Client) error) error {
	...
	user := neturl.UserPassword(connection.Config.Global.User, connection.Config.Global.Password)
	var err error
	...
	defer func() {
		err = c.Logout(ctx)
		if err != nil {
			framework.ExpectNoError(err, "withTagsClient, login error: %v", err)
		}
	}()
	return f(c)
}
...
func retrieveZoneInformationForNode(nodeName string, connection *vSphere,
	...
	err := withTagsClient(ctx, connection, func(c *rest.Client) error {
	...
	if err != nil {
		framework.ExpectNoError(err, "failed to retrieve Zone information: %v", err)
	}
	return zones
}
...
func (nm *NodeMapper) GenerateZoneToDatastoreMap() error {
	...
	for _, nodeInfo := range nameToNodeInfo {
		vc := nodeInfo.vSphere.Config.Global.VCenterHostname
