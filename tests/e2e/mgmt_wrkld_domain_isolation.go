...
var _ bool = ginkgo.Describe("[domain-isolation] Management-Workload-Domain-Isolation", func() {
	...
	*/
	...
	ginkgo.It("Out of band operations on VM service VM", func() {
		...
		usr := GetAndExpectStringEnvVar(vmMigrationUserName)
		pwd := GetAndExpectStringEnvVar(vmMigrationUserPwd)
		...
		sharedSc, err := client.StorageV1().StorageClasses().Get(ctx, sharedStoragePolicyName, metav1.GetOptions{})
		if !apierrors.IsNotFound(err) {
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}
		...
		isMigrateSuccess, _ := migrateVmsToDatastore(svcMasterIp, sshClientConfig, zone3DsName, []string{vm.Name},
			dcName, usr, pwd)
		...
		isMigrateSuccess2, _ := migrateVmsToDatastore(svcMasterIp, sshClientConfig, zone2DsName, []string{vm2.Name},
			dcName, usr, pwd)
