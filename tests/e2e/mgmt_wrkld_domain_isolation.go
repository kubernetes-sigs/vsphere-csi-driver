...
var _ bool = ginkgo.Describe("[domain-isolation] Management-Workload-Domain-Isolation", func() {
	...
	*/
	...
	ginkgo.It("TC1Verifying volume creation and PV affinities with svc namespace tagged to zonal-2 policy, "+
	...
	/*
	   ...
	   2. Create a StatefulSet with 3 replicas, using the storage policy from step #1 and configuring Immediate Binding mode.
	   3. Wait for the StatefulSet PVCs to reach the "Bound" state and the StatefulSet Pods to reach the "Running" state.
	   4. Verify the StatefulSet PVC annotations and the PVs affinity details. Expected to get the affinity of zone-1 on PV.
	   5. Verify the StatefulSet Pod's node annotation.
	   6. Perform cleanup by deleting the Pods, Volumes, and Namespace.
	*/

	ginkgo.It("TC1.1Verify workload creation when wcp namespace is tagged to zone-1 mgmt domain and "+
	...
	*/
	...
	ginkgo.It("TC2Verify workload creation when the WCP namespace is tagged to zone-2 workload domain "+
	...
	/*
	   Testcase-4
	   Create, restore, and delete dynamic snapshots, along with workload/volume creation, while adding and removing zones from the namespace in between
	...
	*/
	...
	ginkgo.It("TC4Create, restore, and delete dynamic snapshot, along with workload/volume creation, "+
		...
		*/
		...
		ginkgo.By("Create a WCP namespace and tag it to zone-2 and zone-3 wrkld domains using storage policy compatible to all zones")
		...
		// output := readFileFromPod(namespace, dep.Name, filePathPod1)
		// gomega.Expect(strings.Contains(output, "Hello message from Pod1")).NotTo(gomega.BeFalse())

		// writeDataOnFileFromPod(namespace, dep.Name, filePathPod1, "Hello message from test into Pod1")
		// output = readFileFromPod(namespace, dep.Name, filePathPod1)
		// gomega.Expect(strings.Contains(output, "Hello message from test into Pod1")).NotTo(gomega.BeFalse())

		// ginkgo.By("Verify the volume is accessible and Read/write is possible")
		// output2 := readFileFromPod(namespace, dep.Name, filePathPod1)
		...
		ginkgo.By("Perform scaling operation on statefulset. Increase the replica count to 6")
		err = performScalingOnStatefulSetAndVerifyPvNodeAffinity(ctx, client,
			3, 0, statefulset, true, namespace, allowedTopologies, true, false, false)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
