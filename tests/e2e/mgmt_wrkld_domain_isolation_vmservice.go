...
var _ bool = ginkgo.Describe("[domain-isolation-vmsvc] Domain-Isolation-VmServiceVm", func() {
	...
	*/
	...
	/*
			Multiple dynamic volume creation
		Vm Service VM creation
		Multizone → zone-1, zone-2 and zone-3 tagged to WCP namespace
		shared storage policy compatible with above zones
		Immediate Binding mode
		Mark zones for removal
		snapshot
		restore snapshot
		VMs attach

		Test Steps:
		    Create a WCP namespace tagged to workload domain zones 1, 2 and 3 with a zonal storage policy(compatible with zones 1,2 and 3) and assign it to the namespace.
		    Create 2 PVCs(pvc-1, pvc-2) using the above policy. (Policy created using Immediate Binding mode)
		    Verify that PVC reached Boud state.
		    Verify PVC annotation.
		    Verify PV affinity. It should show zone-1, zone-2 and zone-3 topology affinity details.
		    Create 2 VM Service VMs(vm-1, vm-2) and attach them to PVC. vm-1 is attach to pvc-1 and vm-2 is attach to pvc-2.
		    ...
		    Mark zone-2 and zone-3 for removal.
		    Take snapshots of pvc-1 and pvc-2.
		    Verify snapshot is taken successfully and ready to state is set to true.
		    Add new zone zone-4 to the WCP namespace.
		     Restore the snapshot to create new volumes (pvc-3, pvc-4).
		    Verify that new PVCs reached Boud state.
		    Verify PVC annotation.
		    Verify PV affinity. It should show zone-1, zone-2 and zone-3 topology affinity details.
		     Attach pvc-4 to vm-1 and pvc-3 to vm-2.
			 Verify attachment of volume to vm went successful.
		    Perform cleanup: Delete VM, PVC and Namespace.


	*/

	ginkgo.It("TC4VM creations when zone is marked for removal", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		*/
		...
		ginkgo.By("Edit vm2 spec and attach restorepvc1 to vm2")
		vm2, err = getVmsvcVM(ctx, vmopC, vm2.Namespace, vm2.Name) // refresh vm info
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
