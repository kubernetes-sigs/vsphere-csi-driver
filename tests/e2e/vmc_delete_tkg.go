...
var _ = ginkgo.Describe("Delete TKG", func() {
	...
	*/
	...
	ginkgo.It("[delete-tkg-vmc] Delete TKG", func() {
		...
		defer func() {
			err = clientNewGc.CoreV1().PersistentVolumeClaims(namespaceNewGC).Delete(ctx, pvclaim.Name, *metav1.NewDeleteOptions(0))
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		deleteTKG(vmcWcpHost, wcpToken, tkg_cluster)
