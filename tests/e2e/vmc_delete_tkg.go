...
var _ = ginkgo.Describe("Delete TKG", func() {
	...
	*/
	...
	ginkgo.It("[delete-tkg-vmc] Delete TKG", func() {
		...
		newPod, err := createPod(ctx, clientNewGc, namespaceNewGC, nil, pvclaims, admissionapi.LevelBaseline, execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pod, err := createPod(ctx, client, gcNamespace, nil, []*v1.PersistentVolumeClaim{pvcNew}, admissionapi.LevelBaseline, execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
