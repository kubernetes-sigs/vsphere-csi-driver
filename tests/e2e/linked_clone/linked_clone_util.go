...
func CreatePvcPodAndSnapshotMap(ctx context.Context, e2eTestConfig *config.E2eTestConfig, client clientset.Interface, namespace string, storageclass *storagev1.StorageClass, doCreatePod bool, doCreateDep bool) (*corev1.PersistentVolumeClaim, []*corev1.PersistentVolume, *snapV1.VolumeSnapshot) {
	...
	if doCreatePod || doCreateDep {
		pod, _ := k8testutil.CreatePodForPvc(ctx, e2eTestConfig, client, namespace, []*corev1.PersistentVolumeClaim{pvclaim}, doCreatePod, doCreateDep)
...
*/
func CreateVolumeSnapshotMap(ctx context.Context, e2eTestConfig *config.E2eTestConfig, namespace string, pvclaim *corev1.PersistentVolumeClaim, pv []*corev1.PersistentVolume, diskSize string) (*snapV1.VolumeSnapshot, *snapV1.VolumeSnapshotContent) {
	// Create snapshot class and dynamic snapshot
	volumeSnapshotClass, volumeSnapshot, snapshotContent := csisnapshot.CreateVolumeSnapshot(ctx, e2eTestConfig, namespace, pvclaim, pv, diskSize)
...
func Cleanup(ctx context.Context, client clientset.Interface, e2eTestConfig *config.E2eTestConfig, namespace string) {
	...
	for i := 0; i < len(snapToDelete); i++ {
		...
		csisnapshot.DeleteVolumeSnapshotWithPandoraWait(ctx, snapc, namespace, snapToDelete[0].Name, constants.DefaultPandoraSyncWaitTime)
	...
	for i := 0; i < len(snapContentToDelete); i++ {
		err := csisnapshot.DeleteVolumeSnapshotContent(ctx, snapContentToDelete[0], snapc, constants.DefaultPandoraSyncWaitTime)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	}
