...
var _ = ginkgo.Describe("[multivc-sitedown] MultiVc-SiteDown", func() {
	...
	*/
	...
	ginkgo.It("Bring down few esx on VC2 availability zones in a multi-vc setup", ginkgo.Label(p1,
		...
		storageclass, err := createStorageClass(client, nil, nil, allowedTopologies, "", "", false, "nginx-sc")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
	...
	*/
	...
	ginkgo.It("Bring down few esx on VC1 availability zones in a multi-vc setup", ginkgo.Label(p1,
		...
		storageclass, err := createStorageClass(client, nil, scParameters, nil, "", "", false, "nginx-sc")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			err := client.StorageV1().StorageClasses().Delete(ctx, storageclass.Name, *metav1.NewDeleteOptions(0))
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("Bring down full site VC2 in a multi setup", ginkgo.Label(p1,
		...
		storageclass, err := createStorageClass(client, nil, scParameters, allowedTopologies, "", "", false,
			"nginx-sc")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			err := client.StorageV1().StorageClasses().Delete(ctx, storageclass.Name, *metav1.NewDeleteOptions(0))
	...
	*/
	...
	ginkgo.It("Bring down full site VC1 in a multi setup", ginkgo.Label(p1,
		...
		ginkgo.By("Create SC with availability zone in VC1 and VC2")
		storageclass, err := createStorageClass(client, nil, scParameters, allowedTopologies, "", "", false, "nginx-sc")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			err := client.StorageV1().StorageClasses().Delete(ctx, storageclass.Name, *metav1.NewDeleteOptions(0))
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
