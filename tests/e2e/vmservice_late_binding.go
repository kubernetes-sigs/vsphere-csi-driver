...
var _ bool = ginkgo.Describe("[vmsvc] VM-Service-VM-LateBinding", func() {
	...
	*/
	ginkgo.It("Attach-restoredPVC-UsingImmediateBindingStorageclass-to-ExistingVMServiceVM", ginkgo.Label(p0, block, wcp, vmsvc, vc901), func() {
		...
		// match := verifyPvcsAreAttachedToVmsvcVmV5(ctx, vmopC, vm1v5, []*v1.PersistentVolumeClaim{pvc1})
		// gomega.Expect(match).To(gomega.BeTrue())
		// match = verifyPvcsAreAttachedToVmsvcVmV5(ctx, vmopC, vm1v5, []*v1.PersistentVolumeClaim{pvclaim2})
		// gomega.Expect(match).To(gomega.BeTrue())
