...
var _ = ginkgo.Describe("[rwm-csi-tkg] Basic File Volume Provision Test", ginkgo.Label(p0, tkg, file, vc70), func() {
	...
	*/
	ginkgo.It("Verify Basic RWX volume provision", func() {
		...
		persistentvolumes, err := WaitForPVClaimBoundPhase(ctx, client,
			adminClient, []*v1.PersistentVolumeClaim{pvclaim}, framework.ClaimProvisionTimeout)
