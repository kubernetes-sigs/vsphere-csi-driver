...
var _ = ginkgo.Describe("[csi-guest] Volume Expansion Test", func() {
	...
	*/
	...
	ginkgo.It("verify offline block volume expansion succeeds when GC CSI pod is down "+
		...
		// Create a Pod to use this PVC, and verify volume has been attached.
		ginkgo.By("Creating pod to attach PV to the node")
		pod, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvc}, admissionapi.LevelBaseline, execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func createStaticPVandPVCandPODinGuestCluster(client clientset.Interface,
	...
	// Create a Pod to use this PVC, and verify volume has been attached.
	ginkgo.By("Creating pod to attach PV to the node")
	pod, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{gcPVC}, admissionapi.LevelBaseline, execCommand)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
