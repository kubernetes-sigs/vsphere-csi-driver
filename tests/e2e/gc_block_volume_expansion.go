...
func verifyPvcRequestedSizeUpdateInSupervisorWithWait(pvcName string, size resource.Quantity) (bool, error) {
	var b bool
	waitErr := wait.PollUntilContextTimeout(context.Background(), resizePollInterval, totalResizeWaitPeriod, true, func(ctx context.Context) (bool, error) {
...
func waitForSvcPvcToReachFileSystemResizePendingCondition(ctx context.Context, svcPvcName string, timeout time.Duration) error {
