...
var _ = ginkgo.Describe("Prevent duplicate cluster ID", func() {
	...
	*/
	ginkgo.It("[csi-block-vanilla][csi-file-vanilla] Generate cluster id and then set cluster id "+
		...
		recreateVsphereConfigSecret(client, ctx, vCenterUIUser, vCenterUIPassword, csiNamespace, vCenterIP,
			"cluster1", vCenterPort, dataCenter, csiReplicas)
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = fpod.WaitForPodsRunningReady(client, csiNamespace, int32(csipods.Size()), 0, pollTimeoutShort*2, nil)
		gomega.Expect(err).To(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[csi-block-vanilla][csi-file-vanilla] Recreate vsphere config secret multiple"+
		" times", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		verifyVolumeMetadataOnDeployments(ctx, client, deployment, namespace, nil, nil,
			nil, "")
		checkClusterIdValueOnWorkloads(&e2eVSphere, client, ctx, namespace, clusterID)
