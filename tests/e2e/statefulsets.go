...
var _ = ginkgo.Describe("statefulset", func() {
		...
		for _, sspod := range ssPodsBeforeScaleDown.Items {
			...
			for _, volumespec := range sspod.Spec.Volumes {
				if volumespec.PersistentVolumeClaim != nil {
					...
					pvclaim, err = expandPVCSize(pvclaim, newSize, testClient)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					gomega.Expect(pvclaim).NotTo(gomega.BeNil())

					ginkgo.By("Waiting for file system resize to finish")
					_, err = waitForFSResize(pvclaim, testClient)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[csi-block-vanilla] [csi-supervisor] ListVolumeResponse "+
		...
		scaledownCSIDriver, err := scaleCSIDriver(ctx, testClient, namespace, 1)
		...
		defer func() {
			...
			success, err := scaleCSIDriver(ctx, testClient, namespace, 3)
		...
		_, _, err = getCSIPodWhereListVolumeResponseIsPresent(ctx, testClient, sshClientConfig,
		...
		_, _, err = getCSIPodWhereListVolumeResponseIsPresent(ctx, testClient, sshClientConfig, containerName, logMessage, nil)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
