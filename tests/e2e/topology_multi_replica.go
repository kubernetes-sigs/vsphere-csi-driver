	...
	func() {
		...
		ginkgo.It("Volume provisioning when CSI Provisioner is deleted during statefulset creation", func() {
			...
			for i := 0; i < len(statefulSets); i++ {
				...
				if windowsEnv{
					...
					statefulPodList := fss.GetPodList(client, statefulSets[i])
		...
		*/
		...
		ginkgo.It("Verify dynamic provisioning with Thick policy when CSI-provisioner goes down", func() {
			...
			csi_controller_pod, k8sMasterIP, err := getK8sMasterNodeIPWhereControllerLeaderIsRunning(ctx,
				client, sshClientConfig, controller_name)
			framework.Logf("K8sMasterNodeIP is '%v'",k8sMasterIP)
			framework.Logf("CSI-Provisioner is running on elected Leader Pod %s "+
				"which is running on master node %s", csi_controller_pod, k8sMasterIP)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		*/
		ginkgo.It("Verify behaviour when CSI syncer is deleted and check fullsync", func() {
			...
			if windowsEnv {
				StaticPod, err = createPod(client, namespace, nil, staticPvcClaims, false, windowsCommand)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			} else {
				StaticPod, err = createPod(client, namespace, nil, staticPvcClaims, false, "")
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			var dynamicPod *v1.Pod
			if windowsEnv {
				dynamicPod, err = createPod(client, namespace, nil, pvclaims, false, windowsCommand)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			} else {
				dynamicPod, err = createPod(client, namespace, nil, pvclaims, false, "")
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		//	newstaticPod, err := createPod(client, namespace, nil, newStaticPvcClaims, false, "")
		    var newstaticPod *v1.Pod
			if windowsEnv {
				newstaticPod, err = createPod(client, namespace, nil, newStaticPvcClaims, false, windowsCommand)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			} else {
				newstaticPod, err = createPod(client, namespace, nil, newStaticPvcClaims, false, "")
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			var newDynamicPod *v1.Pod
			if windowsEnv {
				newDynamicPod, err = createPod(client, namespace, nil, pvclaims, false, windowsCommand)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			} else {
				newDynamicPod, err = createPod(client, namespace, nil, pvclaims, false, "")
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		/*
			...
			Steps//
			Volume provisioning when CSI proxy is down deleted during statefulset creation
			
		*/

		ginkgo.It("Volume provisioning when CSI proxy is down during statefulset creation", func() {
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()
			controller_name = "csi-provisioner"
			sts_count = 3
...
func createParallelStatefulSetSpec(namespace string, no_of_sts int) []*appsv1.StatefulSet {
	...
	for i := 0; i < no_of_sts; i++ {
		...
		statefulset.Spec.VolumeClaimTemplates[len(statefulset.Spec.VolumeClaimTemplates)-1].
			Annotations["volume.beta.kubernetes.io/storage-class"] = "example-windows-sc"
