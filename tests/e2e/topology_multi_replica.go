	...
	func() {
		...
		ginkgo.It("Volume provisioning when CSI Provisioner is deleted during statefulset creation", func() {
			...
			for i := 0; i < len(statefulSets); i++ {
				...
				if i == 2 {
					...
					err = execDockerPauseNKillOnContainer(ctx, sshClientConfig, k8sMasterIP, container_name, k8sVersion)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
				}
			}
			wg.Wait()
			...
			defer func() {
				...
				for i := 0; i < len(pvclaimsList); i++ {
					...
					framework.ExpectNoError(fpv.WaitForPersistentVolumeDeleted(ctx, client,
						pv.Name, poll, framework.ClaimProvisionTimeout))
					err = e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, pv.Spec.CSI.VolumeHandle)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
				}
			}()
		...
		*/
		ginkgo.It("Verify behaviour when CSI syncer is deleted and check fullsync", func() {
			...
			err = e2eVSphere.waitForCNSVolumeToBeCreated(ctx, staticPv.Spec.CSI.VolumeHandle)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			err = fpv.WaitOnPVandPVC(ctx, client, framework.NewTimeoutContext(), namespace, staticPv,
				newStaticPvclaim)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			defer func() {
