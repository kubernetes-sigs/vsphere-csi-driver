	...
	func() {
		...
		*/
		...
		ginkgo.It("Verify behaviour when CSI-resizer deleted and VSAN-Health "+
			...
			for i := 0; i < len(pvclaimsList); i++ {
				...
				pod, err := createPod(ctx, client, namespace, nil, pvclaims, admissionapi.LevelBaseline, "")
			...
			*/
			...
			for i := 0; i < len(pvclaimsList); i++ {
				...
				deployment, err := createDeployment(ctx, client, deploymentReplicaCount, lables,
					nil, namespace, pvclaims, "", admissionapi.LevelBaseline, nginxImage)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				/* Verify PV nde affinity and that the pods are running on appropriate nodes
				for each StatefulSet pod */
			...
			defer func() {
				...
				for i := 0; i < len(pvclaimsList); i++ {
					...
					err = fpv.DeletePersistentVolumeClaim(ctx, client, pvclaimsList[i].Name, namespace)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					framework.ExpectNoError(fpv.WaitForPersistentVolumeDeleted(ctx, client, pv.Name, poll, framework.ClaimProvisionTimeout))
					err = e2eVSphere.waitForCNSVolumeToBeDeleted(pv.Spec.CSI.VolumeHandle)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
				}
			}()
		...
		*/
		ginkgo.It("Verify behaviour when CSI syncer is deleted and check "+
			...
			StaticPod, err := createPod(ctx, client, namespace, nil, staticPvcClaims, admissionapi.LevelBaseline, "")
			...
			dynamicPod, err := createPod(ctx, client, namespace, nil, pvclaims, admissionapi.LevelBaseline, "")
			...
			newstaticPod, err := createPod(ctx, client, namespace, nil, newStaticPvcClaims, admissionapi.LevelBaseline, "")
			// verify volume is attached to the node
			ginkgo.By(fmt.Sprintf("Verify volume: %s is attached to the node: %s",
