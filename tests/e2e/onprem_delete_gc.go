...
var _ = ginkgo.Describe("Delete GC Onprem", func() {
	f := framework.NewDefaultFramework("onprem-delete-tkg")
	var (
	...
	/*
		Test to Delete TKG on Onprem Guest cluster
		Steps
			1. Create SC
	...
	*/
	...
	ginkgo.It("[csi-guest] Deleting GC onprem with Root user", func() {
		var err error
		var wcpToken string
		...
		err = deleteTKG(wcpHost, wcpToken, onpremrootTKG)
		fmt.Print("Delete TKG is successfull", err)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		deleteGC = true
