	...
	*/
	ginkgo.It("Dynamic PVC -  Zonal storage and Immediate binding", func() {
		...
		if isStorageQuotaFSSEnabled {
			...
			storagepolicyquota_pvc_after_cleanup, pvc_reservedQuota_after_cleanup := getStoragePolicyQuotaForSpecificResourceType(ctx,
				restConfig, storageclass.Name, svNamespace, volExtensionName)
		...
		*statefulset.Spec.Replicas = 3
		...
		defer func() {
			...
			if isStorageQuotaFSSEnabled {
				...
				storagepolicyquota_pvc_after_cleanup, pvc_reservedQuota_after_cleanup := getStoragePolicyQuotaForSpecificResourceType(ctx,
					restConfig, zonalPolicy, svNamespace, volExtensionName)
				framework.Logf("volume ********** storagepolicyquota_pvc_after_cleanup :%v, pvc_reservedQuota_after__cleanup: %v PolicyQuota",
					storagepolicyquota_pvc_after_cleanup, pvc_reservedQuota_after_cleanup)
