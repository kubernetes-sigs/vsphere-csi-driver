...
var _ = ginkgo.Describe("[csi-tkgs-ha] Tkgs-HA-SanityTests", func() {
	...
	*/
	...
	/*
		...
		5. Wait for the PODs to reach running state - make sure Pod scheduled on appropriate nodes
			preset in the availability zone
		6. Describe SVC-PV , and GC-PV  and verify node affinity, make sure appropriate node affinity gets added
		7. Delete the above statefullset
		8. Create New statefullset with PODAffinity rules set
		9. Wait for all the PVC's and POD's to come up
		7. Scale up the statefulset replica to 5 , and validate the node affinity on
		   the newly create PV's and annotations on PVC's
		8. Validate the CNS metadata
		9. Scale down the sts to 0
		10.Delete Statefullset,PVC,POD,SC
	*/
	ginkgo.It("Validate statefullset creation with  POD affinity and POD Anti affinity", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		ginkgo.By("CNS_TEST: Running for GC setup")
		...
		*statefulset.Spec.Replicas = 3
		...
		ginkgo.By("Create Statefullset with PodAntiAffinity")
		cleanupsts = true
		CreateStatefulSet(namespace, statefulset, client)
		replicas := *(statefulset.Spec.Replicas)
		...
		defer func() {
			if cleanupsts {
				framework.Logf("cleaning up statefullset with podAtiAffinity")
				cleaupStatefullset(client, ctx, namespace, statefulset)
			}
		}()
		...
		ginkgo.By("Delete Statefullset with PodAntiAffinity")
		cleaupStatefullset(client, ctx, namespace, statefulset)
		cleanupsts = false
		...
		*statefulset.Spec.Replicas = 3
		framework.Logf("Statefulset spec: %v", statefulset)
		ginkgo.By("Create Statefullset with PodAffinity")
		CreateStatefulSet(namespace, statefulset, client)
		replicas = *(statefulset.Spec.Replicas)

		defer func() {
			framework.Logf("cleaning up statefullset with podAffinity")
			cleaupStatefullset(client, ctx, namespace, statefulset)
		}()
	...
	*/
	ginkgo.It("Verify volume provisioning after VC reboot using zonal storage", func() {
		...
		replicas = 5
		framework.Logf(fmt.Sprintf("Increase statefullset %v to number of Replica: %v",
	...
	/*
		...
		10. Delete pod, gc1-pv and gc1-pvc and svc pvc.
	*/
	ginkgo.It("Static volume provisioning across GC, dynamic PVCs and delete GC namespace", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		pv := getPvFromClaim(client, svPvclaim.Namespace, svPvclaim.Name)
		pvUID := string(pv.UID)
