...
var _ = ginkgo.Describe("raw block volume support", func() {
	...
	*/
	ginkgo.It("[csi-block-vanilla] [csi-guest] [csi-block-vanilla-parallelized] "+
		...
		verifyDataOnRawBlockVolume(namespace, pod.Name, devicePath, "This is first write..", 0)
		...
		ginkgo.By(fmt.Sprintf("Write and read new data on raw volume attached to: %v at path %v", pod.Name,
			pod.Spec.Containers[0].VolumeDevices[0].DevicePath))
		verifyDataOnRawBlockVolume(namespace, pod.Name, devicePath, "This is second write..", 0)
	...
	*/
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] [csi-guest] "+
		...
		ginkgo.By(fmt.Sprintf("Write and read data on raw volume attached to: %v at path %v", pod.Name,
			pod.Spec.Containers[0].VolumeDevices[0].DevicePath))
		verifyDataOnRawBlockVolume(namespace, pod.Name, devicePath, "This is write before expansion", 0)
	...
	*/
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] [csi-guest] "+
		...
		ginkgo.By(fmt.Sprintf("Write and read data on raw volume attached to: %v at path %v", pod.Name,
			pod.Spec.Containers[0].VolumeDevices[0].DevicePath))
		verifyDataOnRawBlockVolume(namespace, pod.Name, devicePath, "This is write before offline expansion", 0)
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] Verify snapshot dynamic provisioning workflow with raw block volume", func() {
		...
		verifyDataOnRawBlockVolume(namespace, pod1.Name, devicePath, "This is write before taking snapshot", 0)
		...
		readDataFromRawBlockVolume(namespace, pod2.Name, devicePath, "This is write before taking snapshot")
		ginkgo.By(fmt.Sprintf("Write and read data on restored volume attached to: %v at path %v", pod2.Name,
			pod2.Spec.Containers[0].VolumeDevices[0].DevicePath))
		verifyDataOnRawBlockVolume(namespace, pod2.Name, devicePath, "This is write to volume restored from snapshot", 0)
