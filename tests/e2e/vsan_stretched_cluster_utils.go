...
func powerOffHostParallel(ctx context.Context, hostsToPowerOff []string) {
	...
	_ = vMPowerMgmt(tbinfo.user, tbinfo.location, tbinfo.podname, hostlist, false)
...
func powerOnHostParallel(hostsToPowerOn []string) {
	...
	_ = vMPowerMgmt(tbinfo.user, tbinfo.location, tbinfo.podname, hostlist, true)
	//gomega.Expect(err).NotTo(gomega.HaveOccurred())
	for _, host := range hostsToPowerOn {
...
func changeLeaderOfContainerToComeUpOnMaster(ctx context.Context, client clientset.Interface,
	...
	waitErr := wait.PollUntilContextTimeout(ctx, healthStatusPollInterval, pollTimeout*6, true,
		func(ctx context.Context) (bool, error) {
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			framework.Logf("Len of csi podList items : %v", len(csipods.Items))
			framework.Logf("Size of csi podList items : %v", csipods.Size())
			defer ginkgo.GinkgoRecover()
...
// from a volume created in supervsior cluster concurrently
...
// createCNSRegisterVolumeInParallel registers FCDs in CNS by calling
// createCNSRegisterVolume API concurrently
