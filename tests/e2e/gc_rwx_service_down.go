...
var _ = ginkgo.Describe("File Volume Test on Service down", ginkgo.Label(p1, tkg, file, negative, vc70), func() {
	...
	*/
	ginkgo.It("[rwm-csi-tkg] Verify create a PVC while CNS is down", func() {
		...
		pvArray, err := WaitForPVClaimBoundPhase(ctx, client, adminClient, []*v1.PersistentVolumeClaim{pvclaim},
		...
		err = waitAndVerifyCnsVolumeMetadata4GCVol(ctx, fcdIDInCNS, pvcNameInSV, pvclaim, pvArray[0], nil)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[rwm-csi-tkg] Verify label update a PVC while CNS is down", func() {
		...
		sc, pvclaim, err := createPVCAndStorageClass(ctx, client, adminClient,
