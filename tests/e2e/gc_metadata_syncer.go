...
var _ = ginkgo.Describe("[csi-guest] pvCSI metadata syncer tests", func() {
	...
	// 12. Delete PVC.
	ginkgo.It("[vks-exp-nonprod] Validate PV and PVC labels are updated/deleted "+
		"on CNS", ginkgo.Label(p0, block, tkg, vc70), func() {
		var err error
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	// 8. Delete PVC in GC.
	ginkgo.It("[stable-pq-vks] Verify CnsVolumeMetadata updated after "+
	...
	// 11. Delete PVC.
	ginkgo.It("[stable-pq-vks] Verify labels are not updated on "+
