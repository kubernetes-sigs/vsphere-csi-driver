...
var _ bool = ginkgo.Describe("[domain-isolation-negative] Management-Workload-Domain-Isolation-negative", func() {
	...
	/*
		...
		11. Perform cleanup
	*/
	ginkgo.It("Verify workloads with vsan-health down", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	/*
		...
		10. Perfrom cleanup
	*/
	ginkgo.It("Large disk with zone removal", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("Workload creation when WCP is down", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()

		// flag to check the WCP statu
		isWcpServicestopped := false
		...
		go stopWcpWithWg(ctx, vcAddress, &wg)
