...
var _ = ginkgo.Describe("[csi-multi-vc-config-secret] Multi-VC-Config-Secret", func() {
	...
	*/
	...
	ginkgo.It("Test4", func() {
		...
		deleteVsphereConfigSecret(client, ctx, csiNamespace)

		ginkgo.By("Create config secret on a new namespace")
		createVsphereConfigSecret(newNamespace.Name, vsphereCfg, sshClientConfig, masterIp)
		defer func() {
			ginkgo.By("Recreate config secret on a default csi system namespace")
			deleteVsphereConfigSecret(client, ctx, newNamespace.Name)
			createVsphereConfigSecret(csiNamespace, vsphereCfg, sshClientConfig, masterIp)
		}()

		ginkgo.By("Install vsphere CSI driver on different test namespace")
		setNewNameSpaceInCsiYaml(sshClientConfig, masterIp, csiNamespace, newNamespace.Name)
		defer func() {
			ginkgo.By("Revert vsphere CSI driver on a system csi namespace")
			setNewNameSpaceInCsiYaml(sshClientConfig, masterIp, newNamespace.Name, csiNamespace)
		}()
	...
	/* Testcase-5
	    ...
	    5. Reboot any one VC
	    6. Scale up/Scale-down the statefull set
	    Verify the node affinity
	    clean up the data
	*/

	ginkgo.It("Test5", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		stsReplicas = 3
		scaleUpReplicaCount = 7
		...
		vcAddress2 := strings.Split(vsphereCfg.Global.VCenterHostname, ",")[1] + ":" + sshdPort
		username2 := strings.Split(vsphereCfg.Global.User, ",")[1]
		originalPassword2 := strings.Split(vsphereCfg.Global.Password, ",")[1]
		newPassword2 := "E2E-test-password!234"

		ginkgo.By("Changing password on the vCenter VC1 host")
		clientIndex0 := 0
		err = invokeVCenterChangePassword(username1, originalPassword1, newPassword1, vcAddress1, true, clientIndex0)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		originalVCPasswordChanged1 := true

		ginkgo.By("Changing password on the vCenter VC2 host")
		clientIndex1 := 1
		err = invokeVCenterChangePassword(username2, originalPassword2, newPassword2, vcAddress2, true, clientIndex1)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		originalVCPasswordChanged2 := true

		ginkgo.By("Create vsphere-config-secret file with new VC1 and new VC2 password")
		passwordList := strings.Split(vsphereCfg.Global.Password, ",")
		passwordList[0] = newPassword1
		passwordList[1] = newPassword2
		vsphereCfg.Global.Password = strings.Join(passwordList, ",")
		...
		revertToOriginalVsphereConf = true
		defer func() {
			if originalVCPasswordChanged1 {
				ginkgo.By("Reverting the password change")
				err = invokeVCenterChangePassword(username1, newPassword1, originalPassword1, vcAddress1, true,
					clientIndex0)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				originalVCPasswordChanged1 = false
			}

			if originalVCPasswordChanged2 {
				ginkgo.By("Reverting the password change")
				err = invokeVCenterChangePassword(username2, newPassword2, originalPassword2, vcAddress2, true,
					clientIndex1)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				originalVCPasswordChanged2 = false
			}
	...
	4. Untill driver restarts All the workflows will go fine.
	5. Stateful set creation should be successful
	restart the driver
	    There should be error while provisioning volume on the VC on which password has updated
	    Clean up the data
	*/

	ginkgo.It("Test6", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		clientIndex := 0
		stsReplicas = 3
	...
	After re-starting CSI driver it should throw error, should not come to running state untill the error in
	vsphere-secret is fixed
	*/

	ginkgo.It("Test7", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
