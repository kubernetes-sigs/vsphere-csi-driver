...
var _ = ginkgo.Describe("Volume health check", func() {
	...
	ginkgo.It("[ef-vanilla-block][pq-n1-vanilla-block][pq-n2-vanilla-block] [csi-block-vanilla]"+
		"[csi-block-vanilla-parallelized] Verify pvc is not annotated "+
	...
	ginkgo.It("[csi-file-vanilla] [ef-file-vanilla][pq-n1-vanilla-file][pq-n2-vanilla-file]File Vanilla Verify pvc"+
		" is not annotated with health status", ginkgo.Label(p1, file, vanilla, core, vc70), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		scParameters := make(map[string]string)
