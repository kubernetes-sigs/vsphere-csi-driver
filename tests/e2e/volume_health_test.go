...
var _ = ginkgo.Describe("Volume health check", func() {
	...
	ginkgo.It("[csi-supervisor] [csi-guest] Verify health annotation is not added on the pvc which is on "+
		...
		if supervisorCluster {
			...
			if !apierrors.IsNotFound(err) {
				...
				storageclass, err = createStorageClass(client, adminClient, scParameters, nil, "", "", true, nonShareadstoragePolicyName)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
	...
	ginkgo.It("[csi-supervisor] [csi-guest] Verify Volume health after password "+
		...
		if supervisorCluster {
			...
			// Create resource quota.
			createResourceQuota(client, namespace, rqLimit, storagePolicyName)
			sc, pvc, err = createPVCAndStorageClass(ctx, client, adminClient, namespace,
				nil, scParameters, diskSize, nil, "", true, "", storagePolicyName)
		} else if guestCluster {
			ginkgo.By("CNS_TEST: Running for GC setup")
			scParameters[svStorageClassName] = storagePolicyName
			sc, pvc, err = createPVCAndStorageClass(ctx, client, adminClient, namespace, nil, scParameters, diskSize, nil, "", false, "")
		}
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("[csi-supervisor] [csi-guest] If pod pvc becomes inaccessible restart pod and "+
		"check pod status", ginkgo.Label(p2, block, wcp, tkg, disruptive, negative, vc70), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		var statusFlag bool = false
		var pvSVC *v1.PersistentVolume
