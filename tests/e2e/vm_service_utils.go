...
func createTestWcpNs(
	...
	_, statusCode := invokeRestAPI4PostRequests(vcRestSessionId, nsCreationUrl, reqBody)
...
func delTestWcpNs(vcRestSessionId string, namespace string) {
	...
	_, statusCode := invokeRestAPI4DeleteRequests(vcRestSessionId, nsDeletionUrl)
	gomega.Expect(statusCode).Should(gomega.BeNumerically("==", 204))
...
func getSvcId(vcRestSessionId string) string {
	...
	resp, statusCode := invokeRestAPI4GetRequests(vcRestSessionId, svcIdFetchUrl)
...
// createAndOrGetContentlibId4Url fetches ID of a content lib that matches the given URL, if none are found it create a
...
func createAndOrGetContentlibId4Url(vcRestSessionId string, contentLibUrl string, dsMoId string,
	...
	resp, statusCode := invokeRestAPI4PostRequests(vcRestSessionId, contentlbCreationUrl, reqBody)
	...
	gomega.Expect(contentLibId).NotTo(gomega.BeEmpty())
	framework.Logf("Successfully created content library 'csi-vmsvc-test-content-lib' for the test: %v", contentLibId)
...
func getAllContentLibIds(vcRestSessionId string) []string {
	...
	resp, statusCode := invokeRestAPI4GetRequests(vcRestSessionId, contentLibsFetchUrl)
	gomega.Expect(statusCode).Should(gomega.BeNumerically("==", 200))
...
func getContentLib(vcRestSessionId string, libId string) subscribedContentLibBasic {
	...
	resp, statusCode := invokeRestAPI4GetRequests(vcRestSessionId, contentLibFetchUrl)
	gomega.Expect(statusCode).Should(gomega.BeNumerically("==", 200))
...
// invokeRestAPI4GetRequests invokes GET on given VC REST URL using the passed session token and verifies that the
// return status code is 200
func invokeRestAPI4GetRequests(vcRestSessionId string, url string) ([]byte, int) {
	...
	req.Header.Add("vmware-api-session-Id", vcRestSessionId)
...
// invokeRestAPI4PostRequests invokes POST on given VC REST URL using the passed session token and request body
func invokeRestAPI4PostRequests(vcRestSessionId string, url string, reqBody string) ([]byte, int) {
	transCfg := &http.Transport{
		TLSClientConfig: &tls.Config{InsecureSkipVerify: true},
	}
	httpClient := &http.Client{Transport: transCfg}
...
// invokeRestAPI4DeleteRequests invokes DELETE on given VC REST URL using the passed session token
func invokeRestAPI4DeleteRequests(vcRestSessionId string, url string) ([]byte, int) {
	transCfg := &http.Transport{
		TLSClientConfig: &tls.Config{InsecureSkipVerify: true},
	}
	httpClient := &http.Client{Transport: transCfg}
...
func createVmServiceVmWithPvcs(ctx context.Context, c ctlrclient.Client, namespace string,
	...
	vm := vmopv1.VirtualMachine{
		...
		Spec: vmopv1.VirtualMachineSpec{
			...
			ImageName:    vmi,
			ClassName:    vmClassBestEffortSmall,
