...
func formatNVerifyPvcIsAccessibleV4(diskUuid string, mountIndex int, vmIp string) string {
	...
	unmountCommands := []string{
		fmt.Sprintf("sudo umount %s || true", partitionDev),
	}
	...
	partitionCommands := []string{
		fmt.Sprintf("sudo parted --script %s mklabel gpt", partitionDev),
		fmt.Sprintf("sudo parted --script -a optimal %s mkpart primary 0%% 100%%", partitionDev),
		"lsblk -l",
...
// get zone name on which vm is scheduled
func getVMzonev1(ctx context.Context, vm *vmopv4.VirtualMachine) (string, error) {
	var val string
	var labelOk bool

	err := wait.PollUntilContextTimeout(ctx, poll*10, pollTimeoutSixMin, true,
		func(ctx context.Context) (bool, error) {
			vmlabel := vm.GetLabels()
			//vmannotation := vm.GetAnnotations()
			val, labelOk = vmlabel[vmZoneLabel]
			framework.Logf("val: %v, labelOk: %v", val, labelOk)
			framework.Logf("val: %v, labelOk: %v", val, labelOk)

			if !labelOk {
				return false, nil // keep polling until label appears
			}
			return true, nil // found the label
		})
