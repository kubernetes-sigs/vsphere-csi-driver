...
var _ bool = ginkgo.Describe("[domain-isolation] Management-Workload-Domain-Isolation", func() {
	...
	*/
	...
	ginkgo.It("Perform psod on Az3 cluster with zones additon and removal", func() {
		...
		hostsInCluster4 := getHostsByClusterName(ctx, clusterComputeResource, clusterName3)
	...
	*/
	...
	ginkgo.It("VC reboot with multiple concurrent operations", func() {
		...
		errorOccurred = checkEventsforError(client, podList[0].Namespace,
			metav1.ListOptions{FieldSelector: fmt.Sprintf("involvedObject.name=%s", podList[0].Name)}, expectedErrMsgForPod)
		gomega.Expect(errorOccurred).To(gomega.BeTrue())
		...
		err = markZoneForRemovalFromWcpNs(vcRestSessionId, namespace,
			topologyAffinityDetails[topologyCategories[0]][2]) // this will fetch zone-3
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
