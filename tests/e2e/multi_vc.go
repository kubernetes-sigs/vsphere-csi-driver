...
var _ = ginkgo.Describe("[csi-multi-vc-topology] Multi-VC", func() {
		...
		*/
		...
		defer func() {
			...
			for i := 0; i < len(pvclaimsList); i++ {
				...
				err = multiVCe2eVSphere.waitForCNSVolumeToBeDeletedInMultiVC(ctx, pv.Spec.CSI.VolumeHandle)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}()
		defer func() {
	...
	ginkgo.It("Create Deployment pod using SC with allowed topology set to specific VC", func() {
		...
		*/
		...
		defer func() {
			err := fpv.DeletePersistentVolumeClaim(ctx, client, pvclaim.Name, namespace)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			err = multiVCe2eVSphere.waitForCNSVolumeToBeDeletedInMultiVC(ctx, volHandle)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			pvclaim = nil
		}()
	...
	*/
	...
	ginkgo.It("Create SC with invalid allowed topology details", func() {
		...
		gomega.Expect(fpv.WaitForPersistentVolumeClaimPhase(ctx, v1.ClaimBound,
			client, pvc.Namespace, pvc.Name, framework.Poll, framework.ClaimProvisionTimeout)).To(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("Create workload and reboot one of the VC", func() {
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = waitForHostToBeUp(ctx, vCenterHostname[0])
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("Create workloads when VSAN-health is down on VC1", func() {
		...
		*/
		...
		for i := 0; i < len(pvclaimsList); i++ {
			...
			err = multiVCe2eVSphere.waitForLabelsToBeUpdatedInMultiVC(ctx, pv.Spec.CSI.VolumeHandle, labels,
