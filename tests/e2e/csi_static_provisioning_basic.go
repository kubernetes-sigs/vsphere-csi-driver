...
var _ = ginkgo.Describe("Basic Static Provisioning", func() {
	...
	ginkgo.AfterEach(func() {
		...
		if pv != nil {
			...
			framework.ExpectNoError(e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, pv.Spec.CSI.VolumeHandle))
		}
	...
	ginkgo.It("[csi-supervisor] Verify static provisioning when AccessMode is ReadWriteMany or ReadOnlyMany", func() {
		...
		err = createCNSRegisterVolume(ctx, restConfig, cnsRegisterVolume)
		gomega.Expect(waitForCNSRegisterVolumeToGetCreated(ctx,
			restConfig, namespace, cnsRegisterVolume, poll, supervisorClusterOperationsTimeout)).To(gomega.HaveOccurred())
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("[csi-supervisor] static provisioning workflow - "+
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		gomega.Expect(waitForCNSRegisterVolumeToGetCreated(ctx,
			restConfig, namespace, cnsRegisterVolume, poll, pollTimeoutShort)).To(gomega.HaveOccurred())
		cnsRegisterVolumeName := cnsRegisterVolume.GetName()
		framework.Logf(" CNS register volume name : %s", cnsRegisterVolumeName)
	...
	ginkgo.It("[csi-supervisor] Perform static and dynamic provisioning together, "+
		...
		framework.ExpectNoError(e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, pv1.Spec.CSI.VolumeHandle))

		framework.ExpectNoError(fpv.WaitForPersistentVolumeDeleted(ctx, client,
			pv2.Name, poll, supervisorClusterOperationsTimeout))
		framework.ExpectNoError(e2eVSphere.waitForCNSVolumeToBeDeleted(ctx, pv2.Spec.CSI.VolumeHandle))
