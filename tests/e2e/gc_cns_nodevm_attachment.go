...
var _ = ginkgo.Describe("[csi-guest] CnsNodeVmAttachment persistence", func() {
	...
	ginkgo.It("[cf-vks-f] Verify CnsNodeVmAttachements existence in "+
	...
	ginkgo.It("[cf-vks-f] Verify CnsNodeVmAttachements crd existence when pods are created "+
	...
	ginkgo.It("[cf-vks-f] Detach Statefulset testing with default "+
	...
	// Verify volume is detached from VM.
	// Delete PVC in GC.
	ginkgo.It("[cf-vks-f] Create a Pod mounted with multiple PVC", ginkgo.Label(p0, block, tkg, vc70), func() {
		var sc *storagev1.StorageClass
		var pvc *v1.PersistentVolumeClaim
		var err error
		ctx, cancel := context.WithCancel(context.Background())
