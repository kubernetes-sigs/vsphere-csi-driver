...
var _ = ginkgo.Describe("[csi-guest] CnsNodeVmAttachment persistence", func() {
	...
	ginkgo.It("Verify CnsNodeVmAttachements existence in a pod lifecycle", ginkgo.Label(p0, block, tkg, vc70), func() {
	...
	ginkgo.It("Verify CnsNodeVmAttachements crd existence when pods are created concurrently", ginkgo.Label(p0,
		block, tkg, vc70), func() {
		var err error
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		...
		scParameters[svStorageClassName] = storagePolicyName
		sc1, pvc1, err := createPVCAndStorageClass(ctx, client, adminClient, namespace, nil, scParameters, "", nil, "", false, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		sc2, pvc2, err := createPVCAndStorageClass(ctx, client, adminClient, namespace, nil, scParameters, "", nil, "", false, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		pvs1, err := WaitForPVClaimBoundPhase(ctx, client, adminClient,
		...
		pvs2, err := WaitForPVClaimBoundPhase(ctx, client, adminClient,
	...
	ginkgo.It("Detach Statefulset testing with default podManagementPolicy", ginkgo.Label(p0, block, tkg, vc70), func() {

		ginkgo.By("Creating StorageClass for Statefulset")
		scParameters[svStorageClassName] = storagePolicyName
		sc, err := createStorageClass(client, adminClient, scParameters, nil, "", "", false, "nginx-sc")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		defer func() {
	...
	ginkgo.It("Verify PVC is attached to Pods created in corresponding GC", ginkgo.Label(p0, block, tkg, vc70), func() {
		...
		pod, err := createPod(ctx, clientNewGc, namespaceNewGC, nil, []*v1.PersistentVolumeClaim{pvcNew}, false, execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pod1, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim}, false, execCommand)
