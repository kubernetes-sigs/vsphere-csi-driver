...
var _ = ginkgo.Describe("[csi-guest] CnsNodeVmAttachment persistence", func() {
	...
	ginkgo.It("Verify CnsNodeVmAttachements crd existence when pods are created concurrently", ginkgo.Label(p0,
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			err := adminClient.StorageV1().StorageClasses().Delete(ctx, sc1.Name, *metav1.NewDeleteOptions(0))
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}()
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			err := adminClient.StorageV1().StorageClasses().Delete(ctx, sc2.Name, *metav1.NewDeleteOptions(0))
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}()
