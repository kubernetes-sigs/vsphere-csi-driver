...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] [supervisor-snapshot] TC1Verify snapshot dynamic provisioning "+
	...
	*/
	...
	/*
		...
		Create/Delete snapshot via k8s API using VolumeSnapshotContent (Pre-Provisioned Snapshots)
		1. Create a storage class (eg: vsan default) and create a pvc using this sc
		2. The volumesnapshotclass is set to retain
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC16Volume snapshot creation and restoration workflow "+
		...
		var scParameters1 = make(map[string]string)

		storagePolicyName1 := GetAndExpectStringEnvVar(envStoragePolicyNameForSharedDatastores2)
		profileID := e2eVSphere.GetSpbmPolicyID(storagePolicyName1)
		scParameters1[scParamStoragePolicyID] = profileID
		scName = storagePolicyName1
		scParameters1[scParamFsType] = xfsFSType

		// setting resource quota for storage policy tagged to supervisor namespace
		setStoragePolicyQuota(ctx, restConfig, storagePolicyName, namespace, rqLimit)

		ginkgo.By("Create storage class with xfs filesystem")
		storageclass, err := createStorageClass(client, scParameters, nil, "", "", true, scName)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		if supervisorCluster {
			// Create a new StorageClass with a different name and updated parameters
			newStorageClass := storageclass.DeepCopy()
			newStorageClass.Name = storageclass.Name + "-updated"
			newStorageClass.Parameters = scParameters1

			// Create the new StorageClass
			newStorageClass, err := client.StorageV1().StorageClasses().Create(ctx, newStorageClass, metav1.CreateOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}

		ginkgo.By("Create PVC")
		pvclaim, pvs, err := createPVCAndQueryVolumeInCNS(ctx, client, namespace,
			labelsMap, "", diskSize, storageclass, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		if vanillaCluster {
			vmUUID = getNodeUUID(ctx, client, nodeName2)
		} else if guestCluster {
			vmUUID, err = getVMUUIDFromNodeName(nodeName2)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		} else if supervisorCluster {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] [supervisor-snapshot] TC3Volume "+
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Delete the namespace hosting the pvcs and "+
		...
		ginkgo.By("Create PVC")
		pvclaim, persistentVolumes, err := createPVCAndQueryVolumeInCNS(ctx, client, newNamespaceName, labelsMap, "",
			diskSize, storageclass, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC5Delete a non-existent snapshot", ginkgo.Label(p0, block,
		vanilla, wcp, snapshot, tkg, negative), func() {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC7Create snapshots using default "+
		...
		volumeSnapshot, snapshotContent, snapshotCreated,
			snapshotContentCreated, snapshotId, _, err := createDynamicVolumeSnapshotWithoutSnapClass(ctx, namespace, snapc, volumeSnapshotClass,
			pvclaim, volHandle, diskSize, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			if snapshotContentCreated {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] TC8Create Volume from snapshot with "+
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] TC9Snapshot workflow for statefulsets", ginkgo.Label(p0, block,
		vanilla, snapshot, wcp, tkg), func() {
		...
		Error from server (Deleting volume with snapshots is not allowed): admission webhook "validation.csi.vsphere.vmware.com"
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC4Volume deletion with existing snapshots", ginkgo.Label(p0,
		block, vanilla, snapshot, tkg, wcp, stable, negative), func() {
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC10Snapshot workflow for deployments", ginkgo.Label(p0,
		block, vanilla, tkg, snapshot, wcp, stable), func() {
	...
	/*
		...
		7. Cleanup the pvc
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC12Volume offline resize of a volume "+
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot][supervisor-snapshot] TC27Snapshot restore while the Host "+
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] VC reboot with deployment pvcs "+
		...
		dep2, err := createDeployment(ctx, client, 1, labelsMap2, nil, namespace,
			restoredpvclaims, execCommand, false, busyBoxImageOnGcr)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			ginkgo.By("Delete Deployment-2")
			err := client.AppsV1().Deployments(namespace).Delete(ctx, dep2.Name, metav1.DeleteOptions{})
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot][supervsior-snapshot] VC password reset during snapshot creation", ginkgo.Label(p1, block,
		...
		ginkgo.By("Create a dynamic volume snapshot")
		volumeSnapshot1, snapshotContent1, snapshotCreated1,
			snapshotContentCreated1, _, _, err := createDynamicVolumeSnapshot(ctx, namespace, snapc, volumeSnapshotClass,
			pvclaim, volHandle, diskSize, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			if snapshotContentCreated1 {
		...
		var newPassword, originalConf, username, nimbusGeneratedVcPwd, originalPassword string
		var vsphereCfg e2eTestConfig
		var secret *v1.Secret
		var originalVCPasswordChanged bool
		...
		volumeSnapshot3, snapshotContent3, snapshotCreated3,
			snapshotContentCreated3, _, _, err := createDynamicVolumeSnapshot(ctx, namespace, snapc, volumeSnapshotClass,
			pvclaim, volHandle, diskSize, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			if snapshotContentCreated3 {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] TC26Multi-master and snapshot workflow", ginkgo.Label(p1, block,
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] Volume provision and "+
		"snapshot creation/restore on VVOL Datastore", ginkgo.Label(p0, block, vanilla, snapshot, tkg, wcp), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] Volume provision and "+
		"snapshot creation/restore on VMFS Datastore", ginkgo.Label(p0, block, vanilla, snapshot, tkg, wcp), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] Volume provision and "+
		"snapshot creation/restore on NFS Datastore", ginkgo.Label(p0, block, vanilla, snapshot, tkg, wcp), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Volume provision and snapshot creation/restore on "+
		"VSAN2 Datastore", ginkgo.Label(p0, snapshot, tkg, newTest, wcp), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		invokeSnapshotOperationsOnSharedDatastore(client, ctx, namespace, scParameters, snapc, "VSAN2", pandoraSyncWaitTime)
	})

	/*
	   Testcase-13
	   Volume provision and snapshot creation on VSANDIRECT Datastore
	   1. Create a SC for VMFS datastore and provision a PVC
	   2. Create Snapshot class and take a snapshot of the volume
	   3. Cleanup of snapshot, pvc and sc
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Volume provision and snapshot creation/restore on "+
		"vsan direct Datastore", ginkgo.Label(p0, snapshot, tkg, newTest, wcp), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		invokeSnapshotOperationsOnSharedDatastore(client, ctx, namespace, scParameters, snapc, "VSANDIRECT", pandoraSyncWaitTime)
	})
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] [supervisor-snapshot] Scale-up creation of snapshots across multiple "+
	...
	*/
	...
	/*
		PVC → Pod → Snapshot → RestoreVol → Pod →  OnlineResize → Snapshot
	...
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC21Perform online resize on restored volume", ginkgo.Label(p0,
		...
		volumeSnapshotFromRestoreVol, snapshotContentFromRestoreVol, snapshotCreatedFromRestoreVol,
			snapshotContentCreatedFromRestoreVol, snapshotIdFromRestoreVol, _, err := createDynamicVolumeSnapshot(ctx, namespace, snapc,
			volumeSnapshotClass, pvclaim2, volHandle2, newDiskSize, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
	...
	*/
	...
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC22Offline relocation of FCD with snapshots", ginkgo.Label(p0, snapshot, tkg,
		newTest, stable), func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	...
	/*
		...
		6.  Perform Cleanup.
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC18Volume mode conversion", ginkgo.Label(p0, snapshot, tkg, newTest, stable,
		negative), func() {
	...
	*/
	...
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC19Create restore volume snapshot in consistent order", ginkgo.Label(p0, snapshot,
	...
	/*
		...
		8. Perform cleanup.
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC28Detach volume with snapshot", ginkgo.Label(p1, snapshot, tkg, newTest,
		stable), func() {
	...
	/*
		...
		4. At this scale, try a few workflow operations such as below:
			...
			c) Restart services
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Tc23Scale up snapshot creation by increasing the volume counts and "+
	...
	/*
		...
		10. Run cleanup: Delete snapshots, restored-volumes, pods
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC11Max Snapshots per volume on wcp and gc", ginkgo.Label(
		...
		for i := 0; i < noOfSnapshotToCreate; i++ {
			...
			volumeSnapshot, _, _, _, snapshotId, _, err := createDynamicVolumeSnapshot(ctx, namespace, snapc, volumeSnapshotClass,
				pvclaim, volHandle, diskSize, true)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Create dynamic snapshot in GC1 and referring it create pre-provisoned snapshot "+
		...
		ginkgo.By("Create storage class on GC1")
		storageclass, err := client.StorageV1().StorageClasses().Get(ctx, storagePolicyName, metav1.GetOptions{})
		if !apierrors.IsNotFound(err) {
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}

		ginkgo.By("Create PVC on GC1")
		pvclaim, persistentVolumes, err := createPVCAndQueryVolumeInCNS(ctx, client, namespace1.Name, labelsMap, "",
			diskSize, storageclass, true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
	...
	*/
	...
	ginkgo.It("[supervisor-snapshot] TC2Volume restore using a "+
	...
	*/
	...
	ginkgo.It("[supervisor-snapshot] TC20Verify static provisioning workflow with snapshot", ginkgo.Label(p0, block, wcp), func() {
...
func invokeSnapshotOperationsOnSharedDatastore(client clientset.Interface, ctx context.Context, namespace string,
	...
	} else if sharedDatastoreType == "VSAN2" {
		if vanillaCluster {
			sharedDatastoreURL = os.Getenv(envSharedVsan2DatastoreURL)
			if sharedDatastoreURL == "" {
				ginkgo.Skip("Skipping the test because SHARED_VSAN2_DATASTORE_URL is not set. " +
					"This may be due to testbed is not having shared VSAN2 datastore.")
			}
		} else {
			storagePolicyName = GetAndExpectStringEnvVar(envStoragePolicyNameForVsan2Datastore)
		}
	} else if sharedDatastoreType == "VSANDIRECT" {
		if vanillaCluster {
			sharedDatastoreURL = os.Getenv(envSharedVsanDirectDatastoreURL)
			if sharedDatastoreURL == "" {
				ginkgo.Skip("Skipping the test because SHARED_VSANDIRECT_DATASTORE_URL is not set. " +
					"This may be due to testbed is not having shared VSANDIRECT datastore.")
			}
		} else {
			storagePolicyName = GetAndExpectStringEnvVar(envStoragePolicyNameForVsanDirectDatastore)
		}
	...
	ginkgo.By("Create pvc using a dynamic volume snapshot")
	pvclaim2, persistentVolumes2, pod2 := verifyVolumeRestoreOperation(ctx, client,
		namespace, storageclass, volumeSnapshot, diskSize, true)
	volHandle2 := persistentVolumes2[0].Spec.CSI.VolumeHandle
	if guestCluster {
