...
var _ = ginkgo.Describe("[block-vanilla-snapshot] Volume Snapshot Basic Test", func() {
	...
	*/
	ginkgo.It("Verify snapshot static provisioning through K8s API workflow", func() {
		...
		ginkgo.By("Expect claim to provision volume successfullyy ")
		err = fpv.WaitForPersistentVolumeClaimPhase(v1.ClaimBound, client,
			pvclaim.Namespace, pvclaim.Name, framework.Poll, time.Minute)
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to provision volume")
		pvclaims = append(pvclaims, pvclaim)
	...
	*/
	ginkgo.It("Volume restore using snapshot a dynamic snapshot b pre-provisioned snapshot", func() {
		...
		framework.Logf("output3:%s",output3)
		//gomega.Expect(strings.Contains(output3, "Hello message from test into Pod1 before snapshot")).NotTo(gomega.BeFalse())
		...
		framework.Logf("output4:%s",output4)
		//gomega.Expect(strings.Contains(output4, "Hello message from test into Pod1 after snapshot")).NotTo(gomega.BeFalse())
