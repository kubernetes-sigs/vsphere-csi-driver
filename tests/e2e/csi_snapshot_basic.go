...
import (
	...
	foutput "k8s.io/kubernetes/test/e2e/framework/pod/output"
...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] Volume restore using snapshot a dynamic snapshot b "+
		...
		fkubectl.RunKubectlOrDie(namespace, wrtiecmd...)
		output = fkubectl.RunKubectlOrDie(namespace, cmd...)
		...
		cmd2 := []string{"exec", pod2.Name, "--namespace=" + namespace, "--", "/bin/sh", "-c",
			"cat /mnt/volume1/Pod1.html "}
		output2 := fkubectl.RunKubectlOrDie(namespace, cmd2...)
		...
		wrtiecmd2 := []string{"exec", pod2.Name, "--namespace=" + namespace, "--", "/bin/sh", "-c",
			"echo 'Hello message from test into Pod1' > /mnt/volume1/Pod1.html"}
		fkubectl.RunKubectlOrDie(namespace, wrtiecmd2...)
		output2 = fkubectl.RunKubectlOrDie(namespace, cmd2...)
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Volume snapshot creation and restoration workflow "+
		...
		_, err = foutput.LookForStringInPodExec(namespace, pod.Name, []string{"/bin/cat", "/mnt/volume1/fstype"},
		...
		_, err = foutput.LookForStringInPodExec(namespace, pod2.Name, []string{"/bin/cat", "/mnt/volume1/fstype"},
			xfsFSType, time.Minute)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
