...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] [supervisor-snapshot] Verify snapshot dynamic provisioning "+
		...
		storageclass, err := createStorageClass(client, adminClient, scParameters, nil, "", "", false, scName)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			if vanillaCluster {
		...
		if isQuotaValidationSupported && supervisorCluster {
			...
			_, _, quota["pvc_storagePolicyQuotaBefore"], _, quota["pvc_storagePolicyUsageBefore"], _ =
			...
			_, _, quota["snap_storagePolicyQuotaBefore"], _, quota["snap_storagePolicyUsageBefore"], _ =
				getStoragePolicyUsedAndReservedQuotaDetails(ctx, restConfig,
		...
		if isQuotaValidationSupported && supervisorCluster {
			...
			//Verifies Total quota includes PVC and snapshot
			...
			//Verifies PVC quota on both StoragePolicy CR and StoragePolicyUsage CR
			sp_quota_pvc_status, sp_usage_pvc_status := validateQuotaUsageAfterResourceCreation(ctx, restConfig,
				...
				volExtensionName, []string{diskSize}, quota["totalQuotaUsedBefore"],
				quota["pvc_storagePolicyQuotaBefore"], quota["pvc_storagePolicyUsageBefore"], islatebinding)
			gomega.Expect(sp_quota_pvc_status && sp_usage_pvc_status).NotTo(gomega.BeFalse())

			//Verifies snapshot quota on both StoragePolicy CR and StoragePolicyUsage CR
			sp_quota_Snap_status, sp_usage_Snap_status := validateQuotaUsageAfterResourceCreation(ctx, restConfig,
				storageclass.Name, namespace, snapshotUsage,
				snapshotExtensionName, []string{snapshotSizeStr}, quota["totalQuotaUsedBefore"],
				quota["snap_storagePolicyQuotaBefore"], quota["snap_storagePolicyUsageBefore"], islatebinding)
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Volume snapshot creation and restoration workflow "+
		...
		storageclass, err := createStorageClass(client, adminClient, scParameters, nil, "", "", true, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			if vanillaCluster {
				err := client.StorageV1().StorageClasses().Delete(ctx, storageclass.Name,
		...
		pvclaim2, persistentVolumes2, _ := verifyVolumeRestoreOperation(ctx, client, adminClient,
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] [supervisor-snapshot] Volume "+
		...
		if vanillaCluster {
			...
			storageclass1, err = createStorageClass(client, adminClient, scParameters1, nil, "", "", false, scName)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			defer func() {
		...
		pvclaim2, pvs2, _ := verifyVolumeRestoreOperation(ctx, client, adminClient, namespace, storageclass1,
		...
		if isQuotaValidationSupported && supervisorCluster {
			...
			sp_quota_pvc_status, sp_usage_pvc_status := validateQuotaUsageAfterResourceCreation(ctx, restConfig,
				...
				quota["totalQuotaUsedBefore"], quota["pvc_storagePolicyQuotaBefore"],
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Delete the namespace hosting the pvcs and "+
		...
		if vanillaCluster {
			ginkgo.By("Create storage class and PVC")
			storageclass, err = createStorageClass(client, adminClient, scParameters, nil, "", "", false, "")
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			defer func() {
				err := client.StorageV1().StorageClasses().Delete(ctx, storageclass.Name, *metav1.NewDeleteOptions(0))
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] Create Volume from snapshot with "+
		...
		ginkgo.By("Create storage class")
		storageclass, err := createStorageClass(client, adminClient, scParameters, nil, "", "", true, scName)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			if vanillaCluster {
				err := client.StorageV1().StorageClasses().Delete(ctx, storageclass.Name, *metav1.NewDeleteOptions(0))
		...
		_, err = WaitForPVClaimBoundPhase(ctx, client, adminClient, []*v1.PersistentVolumeClaim{pvclaim2},
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] Snapshot workflow for "+
		...
		pvclaim3, persistentVolumes3, pod3 := verifyVolumeRestoreOperation(ctx, client, adminClient,
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] Snapshot workflow for "+
		...
		pvclaim2, err := fpv.CreatePVC(ctx, client, namespace, pvcSpec)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		persistentvolume2, err := WaitForPVClaimBoundPhase(ctx, client, adminClient, []*v1.PersistentVolumeClaim{pvclaim2},
			framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		volHandle2 := persistentvolume2[0].Spec.CSI.VolumeHandle
		gomega.Expect(volHandle2).NotTo(gomega.BeEmpty())
		...
		for i := 0; i < snapshotOpsScale; i++ {
			...
			if i == 1 {
				...
				if vanillaCluster || supervisorCluster {
					...
					err = deleteCsiControllerPodWhereLeaderIsRunning(ctx, client, adminClient, snapshotterContainerName)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		for i := 0; i < snapshotOpsScale; i++ {
			...
			if i == 1 {
				if vanillaCluster {
					...
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					ginkgo.By("Delete elected leader CSi-Controller-Pod where csi-snapshotter is running")
					err = deleteCsiControllerPodWhereLeaderIsRunning(ctx, client, nil, snapshotterContainerName)
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					err = fpod.WaitForPodsRunningReady(ctx, client, csiSystemNamespace, int(csipods.Size()),
						time.Duration(pollTimeoutShort*2))
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] Scale-up creation of snapshots "+
		...
		persistentvolumes, err = WaitForPVClaimBoundPhase(ctx, client, adminClient, pvclaims,
			framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		_, err = WaitForPVClaimBoundPhase(ctx, client, adminClient, pvclaims2, framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Volume restore using dynamic and pre-provisioned snapshot on "+
		...
		pvclaim2, persistentVolumes2, pod := verifyVolumeRestoreOperation(ctx, client, adminClient,
	...
	*/
	...
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Perform online resize on restored volume", ginkgo.Label(p0,
		...
		if guestCluster {
			...
			ginkgo.By("Waiting for controller volume resize to finish")
			err = waitForPvResizeForGivenPvc(pvclaim2, client, adminClient, totalResizeWaitPeriod)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[tkg-snapshot] Dynamic snapshot created in one guest cluster "+
		...
		ginkgo.Label(p2, snapshot, tkg, negative, vc80), func() {
			...
			_, err = WaitForPVClaimBoundPhase(ctx, clientNewGc, adminClient,
				[]*v1.PersistentVolumeClaim{pvclaim2}, framework.ClaimProvisionTimeout)
			gomega.Expect(err).To(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Volume snapshot creation on a file-share volume on a guest "+
		...
		ginkgo.By("Get storage class and create PVC")
		_, pvclaim, err := createPVCAndStorageClass(ctx, client, adminClient,
	...
	*/
	...
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Create restore volume snapshot "+
		...
		pvclaim2, pvs2, pod2 := verifyVolumeRestoreOperation(ctx, client, adminClient,
			namespace, storageclass, volumeSnapshot1, diskSize, true)
		volHandle2 := pvs2[0].Spec.CSI.VolumeHandle
		gomega.Expect(volHandle2).NotTo(gomega.BeEmpty())
		if guestCluster {
		...
		ginkgo.By("Restore volume from snapshot created above")
		pvclaim3, pvs3, pod3 := verifyVolumeRestoreOperation(ctx, client, adminClient,
	...
	*/
	...
	ginkgo.It("[supervisor-snapshot] Verify static provisioning workflow "+
		...
		restConfig, _, profileID := staticProvisioningPreSetUpUtil(ctx, f, client, adminClient, storagePolicyName)
