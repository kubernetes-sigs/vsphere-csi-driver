...
var _ = ginkgo.Describe("[block-vanilla-snapshot] Volume Snapshot Basic Test", func() {
	...
	*/
	ginkgo.It("[tkg-snapshot] TC1Verify snapshot dynamic provisioning workflow", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] Table4TC16Volume snapshot creation and restoration workflow with xfs filesystem", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] TC6Volume restore using snapshot on a different storageclass", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] TC12Delete a non-existent snapshot", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] TC15Create Volume from snapshot with different size", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] TC7Volume deletion with existing snapshots", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] TC17Snapshot workflow for deployments", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	/*
		...
		7. Cleanup the pvc
	*/
	ginkgo.It("[tkg-snapshot] TC20Volume offline resize of a volume having snapshots", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	/*
		...
		7. Cleanup the pvc
	*/
	ginkgo.It("[tkg-snapshot] TC21Volume online resize of a volume having snapshots", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] Table2TC5VC reboot with deployment pvcs having snapshot", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[tkg-snapshot] TC22Volume snapshot creation when resize is in progress", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("Table3TC1Scale-up creation of snapshots across multiple volumes", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
