...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot][supervisor-snapshot] TC9Snapshot workflow for statefulsets", ginkgo.Label(p0, block,
		...
		*statefulset.Spec.Replicas = 2
		...
		ginkgo.By("Delete PVC for the deleted pod")
		...
		ginkgo.By("Restore a PVC with the same name as above from the volume snapshot")
		...
		if guestCluster {
			volHandleOfNewPV = getVolumeIDFromSupervisorCluster(volHandleOfNewPV)
		}
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] Snapshot workflow for deployments", ginkgo.Label(p0,
	...
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Max Snapshots per volume on wcp and gc", ginkgo.Label(
		...
		defer func() {
			for i := 0; i < 33; i++ {
				err = deleteVolumeSnapshotContent(ctx, volumeSnapshotContents[i], snapc, pandoraSyncWaitTime)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[supervisor-snapshot] TC2Volume restore using a "+
		...
		volHandle2 := pvs2[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle2)
		}
		defer func() {
			err := fpv.DeletePersistentVolumeClaim(ctx, client, pvclaim2.Name, namespace)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
