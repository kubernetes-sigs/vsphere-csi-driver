...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC16Volume snapshot creation and restoration workflow "+
		...
		ginkgo.By("Create PVC")
		pvclaim, pvs := createPVCAndQueryVolumeInCNS(ctx, client, namespace,
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Delete the namespace hosting the pvcs and "+
		...
		ginkgo.By("Create PVC")
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, newNamespaceName, labelsMap, "",
			diskSize, storageclass, true)
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot][supervisor-snapshot] TC13Volume online resize of a volume having "+
		...
		ginkgo.By("Create PVC")
		pvclaim, pvs := createPVCAndQueryVolumeInCNS(ctx, client, namespace, labelsMap, "",
			diskSize, storageclass, true)
		volHandle := pvs[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
	...
	*/
	...
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] Create restore volume snapshot in consistent order", ginkgo.Label(p0, snapshot,
		...
		ginkgo.By("Create PVC")
		pvclaim1, pvs1 := createPVCAndQueryVolumeInCNS(ctx, client, namespace, labelsMap, "",
			diskSize, storageclass, true)
	...
	*/
	ginkgo.It("[tkg-snapshot][supervisor-snapshot] TC11Max Snapshots per volume on wcp and gc", ginkgo.Label(
		...
		ginkgo.By("Create PVC")
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, namespace, labelsMap, v1.ReadWriteOnce,
			diskSize, storageclass, true)
		volHandle = persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Create dynamic snapshot in GC1 and referring it create pre-provisoned snapshot "+
		...
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, namespace1.Name, labelsMap, "",
			diskSize, storageclass, true)
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
