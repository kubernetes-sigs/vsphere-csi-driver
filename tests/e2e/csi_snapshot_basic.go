...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] Volume restore using snapshot a dynamic snapshot b "+
		...
		pod, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim2},
		...
		// Create a Pod to use this PVC, and verify volume has been attached
		ginkgo.By("Creating pod to attach PV to the node")
		pod2, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim3}, admissionapi.LevelBaseline,
			execRWXCommandPod1)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Volume snapshot creation and restoration workflow "+
		...
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, namespace, nil, "", diskSize, storageclass, true)
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
		}
		...
		// Create a Pod to use this PVC, and verify volume has been attached
		ginkgo.By("Creating pod to attach PV to the node")
		pod, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim}, admissionapi.LevelBaseline,
			execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pod2, err := createPod(ctx, client, namespace, nodeSelector, []*v1.PersistentVolumeClaim{pvclaim2}, admissionapi.LevelBaseline,
			execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Snapshot workflow for deployments", ginkgo.Label(p0,
		...
		dep, err := createDeployment(ctx, client, 1, labelsMap, nil, namespace,
			[]*v1.PersistentVolumeClaim{pvclaim}, execRWXCommandPod1, admissionapi.LevelBaseline, busyBoxImageOnGcr)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		dep2, err := createDeployment(ctx, client, 1, labelsMap2, nil, namespace,
			[]*v1.PersistentVolumeClaim{pvclaim2}, "", admissionapi.LevelBaseline, busyBoxImageOnGcr)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] VC reboot with deployment pvcs "+
		...
		dep, err := createDeployment(ctx, client, 1, labelsMap, nil, namespace,
			pvclaims, execRWXCommandPod1, admissionapi.LevelBaseline, busyBoxImageOnGcr)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		dep2, err := createDeployment(ctx, client, 1, labelsMap2, nil, namespace,
			restoredpvclaims, "", admissionapi.LevelBaseline, busyBoxImageOnGcr)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] VC password reset during snapshot creation", ginkgo.Label(p1, block,
		...
		ginkgo.By("Reverting the password change")
		err = invokeVCenterChangePassword(ctx, username, nimbusGeneratedVcPwd, originalPassword, vcAddress, false, clientIndex)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Create restore volume snapshot in consistent order", ginkgo.Label(p0, snapshot,
		...
		ginkgo.By("Create Pod")
		pod1, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim1}, admissionapi.LevelBaseline,
			execRWXCommandPod1)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		ginkgo.By("Create Pod")
		pod2, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvclaim2}, admissionapi.LevelBaseline,
			execRWXCommandPod2)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
			ginkgo.By(fmt.Sprintf("Deleting the pod %s in namespace %s", pod2.Name, namespace))
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Create dynamic snapshot in GC1 and referring it create pre-provisoned snapshot "+
		...
		namespace1, err := framework.CreateTestingNS(ctx, f.BaseName, client, map[string]string{
			"e2e-framework": f.BaseName,
		})
		...
		namespace2, err := framework.CreateTestingNS(ctx, f.BaseName, clientNewGc, map[string]string{
			"e2e-framework": f.BaseName,
		})
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Error creating namespace on second GC")
		defer func() {
