...
var _ = ginkgo.Describe("[block-vanilla-snapshot] Volume Snapshot Basic Test", func() {
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Delete the namespace hosting the pvcs and "+
		...
		var snapshotId, snapshotHandle, claimPolicy, snapshotContent2Name string
		...
		if vanillaCluster {
			snapshotContent2Name = snapshotcontent2.ObjectMeta.Name
		} else if guestCluster {
			snapshotContent2Name = svcVolumeSnapshotName
		}
		ginkgo.By("Create a volume snapshot by snapshotcontent")
		volumeSnapshot2, err := snapc.SnapshotV1().VolumeSnapshots(namespace2Name).Create(ctx,
			getVolumeSnapshotSpecByName(namespace2Name, "static-vs-cns",
				snapshotContent2Name), metav1.CreateOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		framework.Logf("Volume snapshot name is : %s", volumeSnapshot2.Name)
		snapshotCreated2 := true
	...
	*/
	...
	/*
		...
		2. Create Pre-Provisioned and Dynamic Provisioned snapshot on above PVC.
	...
	*/
	ginkgo.It("[tkg-snapshot] Detach volume with snapshot", func() {
		...
		_, staticSnapshot, staticSnapshotContentCreated, staticSnapshotCreated, err := createPreProvisionedSnapshotInGuestCluster(ctx, volumeSnapshot,
		...
		snapshots = append(snapshots, volumeSnapshot, staticSnapshot)
		...
		for i := 0; i < 2; i++ {
			pvclaim2, persistentVolumes2, pod2 := verifyVolumeRestoreOperation(ctx, client,
				namespace, storageclass, snapshots[i], true)
			volHandle2 := persistentVolumes2[0].Spec.CSI.VolumeHandle
			if guestCluster {
				volHandle2 = getVolumeIDFromSupervisorCluster(volHandle2)
			}
	...
	ginkgo.It("[tkg-snapshot] Provisioning of static volume on guest cluster using FCD with snapshot creation", func() {
		...
		volumeSnapshot, snapshotContent, snapshotCreated,
			...
			volumeSnapshotClass, gcPVC, volumeHandle, diskSize)
		...
		ginkgo.By("Delete Preprovisioned snapshot")
		staticSnapshotCreated, staticSnapshotContentCreated, err = deleteVolumeSnapshot(ctx, snapc, namespace,
			staticSnapshot, pandoraSyncWaitTime, volumeHandle, staticSnapshotId)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		ginkgo.By("Delete Dynamic snapshot")
		snapshotCreated, snapshotContentCreated, err = deleteVolumeSnapshot(ctx, snapc, namespace,
			volumeSnapshot, pandoraSyncWaitTime, volumeHandle, snapshotId)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
