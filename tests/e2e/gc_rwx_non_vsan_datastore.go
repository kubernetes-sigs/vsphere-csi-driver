...
var _ = ginkgo.Describe("[rwm-csi-guest] File Volume Provision with Non-VSAN datastore", func() {
	...
	/*
		...
		4. Delete PVC
	*/
	ginkgo.It("Verify RWX ReadWriteMany volume provision on non-vsan datastore", func() {
		...
		gomega.Expect(err).To(gomega.HaveOccurred(), "Volume provision succeeded")
	})
