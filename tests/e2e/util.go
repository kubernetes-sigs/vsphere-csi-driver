...
func verifyPodCreation(f *framework.Framework, ctx context.Context, client clientset.Interface, namespace string,
	pvc *v1.PersistentVolumeClaim, pv *v1.PersistentVolume) {
	defer ginkgo.GinkgoRecover()
	ginkgo.By("Create pod and wait for this to be in running phase")
	pod, err := createPod(ctx, client, namespace, nil, []*v1.PersistentVolumeClaim{pvc},
		admissionapi.LevelPrivileged, "")
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func CreatePodByUserID(ctx context.Context, client clientset.Interface, namespace string,
	nodeSelector map[string]string, pvclaims []*v1.PersistentVolumeClaim, privilegeLvl admissionapi.Level,
	command string, userID int64) (*v1.Pod, error) {
	pod := GetPodSpecByUserID(namespace, nodeSelector, pvclaims, admissionapi.LevelPrivileged, command, userID)
...
func GetPodSpecByUserID(ns string, nodeSelector map[string]string, pvclaims []*v1.PersistentVolumeClaim,
	privilegeLvl admissionapi.Level, command string, userID int64) *v1.Pod {
	if len(command) == 0 {
		command = "trap exit TERM; while true; do sleep 1; done"
	}
	sctxt := fpod.GenerateContainerSecurityContext(privilegeLvl)
	sctxt.RunAsUser = &userID
	podSpec := &v1.Pod{
		TypeMeta: metav1.TypeMeta{
			Kind:       "Pod",
			APIVersion: "v1",
...
func createPod(ctx context.Context, client clientset.Interface, namespace string, nodeSelector map[string]string,
	pvclaims []*v1.PersistentVolumeClaim, privilegeLvl admissionapi.Level, command string) (*v1.Pod, error) {
	pod := fpod.MakePod(namespace, nodeSelector, pvclaims, privilegeLvl, command)
	if windowsEnv {
		var commands []string
...
func getDeploymentSpec(ctx context.Context, client clientset.Interface, replicas int32,
	...
	pvclaims []*v1.PersistentVolumeClaim, command string, privilegeLvl admissionapi.Level, image string) *appsv1.Deployment {
	zero := int64(0)
					...
					Containers: []v1.Container{
						{
							...
							Command:         []string{"/bin/sh"},
							Args:            []string{"-c", command},
							SecurityContext: fpod.GenerateContainerSecurityContext(privilegeLvl),
						},
					},
...
func createDeployment(ctx context.Context, client clientset.Interface, replicas int32,
	podLabels map[string]string, nodeSelector map[string]string, namespace string,
	pvclaims []*v1.PersistentVolumeClaim, command string, privilegeLvl admissionapi.Level, image string) (*appsv1.Deployment, error) {
	if len(command) == 0 {
		command = "trap exit TERM; while true; do sleep 1; done"
	}
	deploymentSpec := getDeploymentSpec(ctx, client, replicas, podLabels, nodeSelector, namespace,
		pvclaims, command, privilegeLvl, image)
	if windowsEnv {
...
func createPodForFSGroup(ctx context.Context, client clientset.Interface, namespace string,
	...
	privilegeLvl admissionapi.Level, command string, fsGroup *int64, runAsUser *int64) (*v1.Pod, error) {
	if len(command) == 0 {
		command = "trap exit TERM; while true; do sleep 1; done"
	}
	if fsGroup == nil {
	...
	pod := fpod.MakePod(namespace, nodeSelector, pvclaims, admissionapi.LevelPrivileged, command)
	pod.Spec.Containers[0].Image = busyBoxImageOnGcr
