...
var _ = ginkgo.Describe("[csi-vcp-mig] VCP to CSI migration syncer tests", func() {
	...
	*/
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] Test VCP-to-CSI migration "+
		...
		verifyDataOnRawBlockVolume(namespace, pod.Name, pvc1_devicePath, "This is write to first VCP volume", 0)
		...
		*rep = 1
		...
		verifyDataOnRawBlockVolume(namespace, pod.Name, pvc2_devicePath, "This is write to second VCP volume", 0)
	...
	// Verify in-line volume creation with raw block volume mode on the migrated node.
	// Steps:
	// 1. Create vmdk1.
	// 2. Create VCP SC1.
	// 3. Enable CSIMigration and CSIMigrationvSphere feature gates on
	...
	//       following annotation - storage.alpha.kubernetes.io/migrated-plugins.
	//    d. Enable scheduling on the node.
	ginkgo.It("Verify in-line volume creation with raw block volume mode on the migrated node", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		ginkgo.By("Creating VCP SC")
		scParams := make(map[string]string)
		...
		ginkgo.By("Create pod1 with inline raw block volume and wait for it to reach Running state")
		volumeIndex := 1
		volumeName := fmt.Sprintf("volume%v", volumeIndex)
		pvc1_devicePath := fmt.Sprintf("%v%v", pod_devicePathPrefix, volumeIndex)
		pod1 := getPodSpecWithInlineVols(namespace, nil, []string{vmdk1})
		pod1.Spec.Containers[len(pod1.Spec.Containers)-1].VolumeMounts = nil
		pod1.Spec.Containers[len(pod1.Spec.Containers)-1].VolumeDevices =
			[]v1.VolumeDevice{
				{
					Name:       volumeName,
					DevicePath: pvc1_devicePath,
		...
		pod1, err = client.CoreV1().Pods(namespace).Create(context.TODO(), pod1, metav1.CreateOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		// Waiting for pod to be running
		err = fpod.WaitForPodNameRunningInNamespace(client, pod1.Name, namespace)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		// get fresh pod info
		pod1, err = client.CoreV1().Pods(namespace).Get(context.TODO(), pod1.Name, metav1.GetOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		podsToDelete = append(podsToDelete, pod1)
...
// getPodSpecWithInlineVols creates a pod spec with given volumes inline
func getPodSpecWithInlineVols(namespace string, nodeSelector map[string]string, vmdks []string) *v1.Pod {
...
func createPodWithInlineVols(ctx context.Context, client clientset.Interface,
	...
	podSpec := getPodSpecWithInlineVols(namespace, nodeSelector, vmdks)
