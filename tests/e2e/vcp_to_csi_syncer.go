...
var _ = ginkgo.Describe("[csi-vcp-mig] VCP to CSI migration syncer tests", func() {
	...
	ginkgo.It("Verify in-line volume creation on the migrated node", func() {
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = waitAndVerifyCnsVolumeMetadata(ctx, crd.Spec.VolumeID, nil, nil, pod1)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("Relocate FCD used by PV provisioned using VCP SC post migration", func() {
		...
		ginkgo.By("Wait and verify CNS entries for all CNS volumes")
		err = waitAndVerifyCnsVolumeMetadata(ctx, volHandle, pvc1, vcpPv, pod)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		ginkgo.By("Wait and verify CNS entries for all CNS volumes")
		err = waitAndVerifyCnsVolumeMetadata(ctx, volHandle, pvc1, vcpPv, nil)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func verifyCnsVolumeMetadataAndCnsVSphereVolumeMigrationCrdForPvcs(ctx context.Context,
	...
	for _, pvc := range pvcs {
		...
		err = waitAndVerifyCnsVolumeMetadata(ctx, crd.Spec.VolumeID, pvc, pv, pod)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	}
}
...
func wait4DeploymentPodsCreation(ctx context.Context,
	c clientset.Interface, dep *appsv1.Deployment) (*v1.PodList, error) {
	var pods *v1.PodList
	var err error
	waitErr := wait.PollUntilContextTimeout(ctx, poll, pollTimeout,
