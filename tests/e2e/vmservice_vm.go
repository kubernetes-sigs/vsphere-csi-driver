...
var _ bool = ginkgo.Describe("[vmsvc] vm service with csi vol tests", func() {
	...
	*/
	ginkgo.It("verify vmservice vm creation with a pvc in its spec", ginkgo.Label(p0,
		...
		pvs, err := WaitForPVClaimBoundPhase(ctx, client, adminClient, []*v1.PersistentVolumeClaim{pvc, staticPvc}, pollTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		pv := pvs[0]
	...
	*/
	ginkgo.It("attach PVC used by one VM to another VM while in use", ginkgo.Label(p1,
		...
		ginkgo.By("Waiting for all claims to be in bound state")
		pvs, err := WaitForPVClaimBoundPhase(ctx, client, adminClient, pvcs, pollTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
