...
var _ bool = ginkgo.Describe("[vmsvc] vm service with csi vol tests", func() {
	...
	*/
	...
	/*
	   ...
	   9   Remove spbm policy attached to test namespace
	*/
	ginkgo.It("static-vm and verify vm creation and validate storagequota", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		var err error
	...
	*/
	ginkgo.It("StorageQuotaValidation-for-VMservice-VMs", func() {
		...
		if isQuotaValidationSupported {
			...
			vm_totalQuotaUsedBeforeAfter, _, vm_storagePolicyQuotaBeforeAfter, _, vm_storagePolicyUsageBeforeAfter, _ :=
				getStoragePolicyUsedAndReservedQuotaDetails(ctx, restConfig,
					storageclass.Name, namespace, vmUsage, vmServiceExtensionName)

			framework.Logf("vm_totalQuotaUsedBeforeAfter: %v, vm_storagePolicyQuotaBeforeAfter: %v, vm_storagePolicyUsageBeforeAfter: %v",
				vm_totalQuotaUsedBeforeAfter, vm_storagePolicyQuotaBeforeAfter, vm_storagePolicyUsageBeforeAfter)
