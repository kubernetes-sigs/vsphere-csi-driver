...
var _ = ginkgo.Describe("[csi-guest] Guest cluster fullsync tests", func() {
	...
	// Delete PVC.
	ginkgo.It("Verify CNS volume is synced with updated GC PV & PVC labels when GC csi-controller pod is down", ginkgo.Label(p0, block, tkg, vc70), func() {
	...
	// Verify entry is updated in CNS.
	// Delete PVC.
	ginkgo.It("Verify CNS volume is synced with updated GC PV & PVC labels after SVC connection is restored", ginkgo.Label(p0, block, tkg, vc70), func() {
		var err error
		var sc *storagev1.StorageClass
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
