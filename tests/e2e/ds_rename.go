	...
	*/
	ginkgo.It("Create PVC and then rename datastore", ginkgo.Label(p0, block, vanilla, vc90), func() {
		...
		for i := 0; i < len(volumeSnapshotList); i++ {
			...
			verifyVolumeRestoreOperation(ctx, client, nil, namespace, sc, volumeSnapshotList[i], diskSize1GB, false)
	...
	*/
	ginkgo.It("Create CSI snapshot while renaming datastore", ginkgo.Label(p0, block, vanilla, vc90), func() {
		...
		for i, volumeSnapshot := range volumeSnapshotList {
			...
			verifyVolumeRestoreOperation(ctx, client, nil, namespace, sc, volumeSnapshot, diskSize1GB, false)
