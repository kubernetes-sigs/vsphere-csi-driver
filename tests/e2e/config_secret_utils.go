...
func govcLoginCmd(vcAddress string) string {
	...
	loginCmd += fmt.Sprintf("export GOVC_URL='https://%s:%s@%s';", vCenterUIUser, vCenterUIPassword, vcAddress)
	return loginCmd
}

// createTestUser util method is used for creating test users
func createTestUser(vcAddress string, sshClientConfig *ssh.ClientConfig, masterIp string,
	testUser string, testUserPassword string) error {
	// create test user
	createUser := govcLoginCmd(vcAddress) + "govc sso.user.create -p " + testUserPassword + " " + testUser
...
func deleteUsersRolesAndPermissions(vcAddress string, sshClientConfig *ssh.ClientConfig,
...
func deleteUserPermissions(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, dataCenter []string, clusters []string, hosts []string,
	vms []string, datastores []string) error {
	err := deleteDataCenterPermissions(vcAddress, sshClientConfig, masterIp, testUser, dataCenter)
	if err != nil {
		return err
	}
	err = deleteHostsLevelPermission(vcAddress, sshClientConfig, masterIp, testUser, hosts)
	if err != nil {
		return err
	}
	err = deleteVMsLevelPermission(vcAddress, sshClientConfig, masterIp, testUser, vms)
	if err != nil {
		return err
	}
	err = deleteClusterLevelPermission(vcAddress, sshClientConfig, masterIp, testUser, clusters)
	if err != nil {
		return err
	}
	for i := 0; i < len(dataCenter); i++ {
		err = deleteDataStoreLevelPermission(vcAddress, sshClientConfig, masterIp, testUser, dataCenter[i],
			datastores)
		if err != nil {
			return err
		}
	}
	err = deleteOtherPermissionsFromTestUser(vcAddress, sshClientConfig, masterIp, testUser)
	if err != nil {
		return err
	}
	return nil
...
func deleteDataCenterPermissions(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, dataCenter []string) error {
	for i := 0; i < len(dataCenter); i++ {
		deleteDataCenterPermissions := govcLoginCmd(vcAddress) + "govc permissions.remove -principal " + testUser +
			" " + dataCenter[i]
...
func deleteHostsLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, hosts []string) error {
	for i := 0; i < len(hosts); i++ {
		deleteHostsLevelPermissions := govcLoginCmd(vcAddress) + "govc permissions.remove -principal " + testUser +
			" " + hosts[i]
...
func deleteVMsLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, vms []string) error {
	for i := 0; i < len(vms); i++ {
		deleteVmsLevelPermissions := govcLoginCmd(vcAddress) + "govc permissions.remove -principal " + testUser +
			" " + vms[i]
...
func deleteClusterLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, clusters []string) error {
	for i := 0; i < len(clusters); i++ {
		deleteClusterLevelPermissions := govcLoginCmd(vcAddress) + "govc permissions.remove -principal " + testUser +
			" " + clusters[i]
...
func deleteDataStoreLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, dataCenter string, datastores []string) error {
	for i := 0; i < len(datastores); i++ {
		deleteDataStoreLevelPermissions := govcLoginCmd(vcAddress) + "govc permissions.remove -principal " +
...
func deleteOtherPermissionsFromTestUser(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string) error {
	deleteOtherPermissions := govcLoginCmd(vcAddress) + "govc permissions.remove -principal " +
		testUser
...
func deleteUserRoles(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string) error {
	deleteRoles := govcLoginCmd(vcAddress) + "govc role.remove CNS-DATASTORE-" + testUser + ";" +
...
func deleteTestUser(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string) error {
	deleteUser := govcLoginCmd(vcAddress) + "govc sso.user.rm " + testUser
...
func createRolesForTestUser(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string) error {
	createRoleCmdFortestUser := govcLoginCmd(vcAddress) + "govc role.create CNS-DATASTORE-" + testUser +
...
func getDataCenterClusterHostAndVmDetails(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string) ([]string, []string, []string, []string, []string, error) {
	// fetch datacenter details
	dataCenters, err := getDataCenterDetails(vcAddress, sshClientConfig, masterIp)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())

	// fetch cluster details
	clusters, err := getClusterDetails(vcAddress, sshClientConfig, masterIp, dataCenters)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())

	// fetch esxi hosts details
	hosts, err := getEsxiHostDetails(vcAddress, sshClientConfig, masterIp, clusters)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())

	// fetch vm details
	vms, err := getVmDetails(vcAddress, sshClientConfig, masterIp, dataCenters)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())

	// fetch datastore details
	datastores, err := getDatastoreDetails(vcAddress, sshClientConfig, masterIp, dataCenters)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func getDataCenterDetails(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string) ([]string, error) {
	var dataCenterList []string
	// fetch datacenter details
	ginkgo.By("Executing command for fetching data center details")
	dataCenter := govcLoginCmd(vcAddress) + "govc datacenter.info | grep -i Name | awk '{print $2}'"
...
func getClusterDetails(vcAddress string, sshClientConfig *ssh.ClientConfig,
	...
	for i := 0; i < len(dataCenter); i++ {
		cluster := govcLoginCmd(vcAddress) + "govc ls /" + dataCenter[i] + "/host"
...
func getEsxiHostDetails(vcAddress string, sshClientConfig *ssh.ClientConfig,
	...
	for i := 0; i < len(cluster); i++ {
		hosts := govcLoginCmd(vcAddress) + "govc ls " + cluster[i] + " " + " | grep 10."
...
func getVmDetails(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, dataCenter []string) ([]string, error) {
	var vmsList []string
	var vmList []string
	// get vm details
	...
	for i := 0; i < len(dataCenter); i++ {
		vms := govcLoginCmd(vcAddress) + "govc ls /" + dataCenter[i] + "/vm" + " " + "| grep 'k8s\\|haproxy'"
...
func getDatastoreDetails(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, dataCenter []string) ([]string, error) {
	var dsList []string
	...
	for i := 0; i < len(dataCenter); i++ {
		ds := govcLoginCmd(vcAddress) + "govc ls /" + dataCenter[i] + "/datastore"
...
func setDataCenterLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, dataCenter string, testUser string) error {
	setPermissionForDataCenter := govcLoginCmd(vcAddress) + "govc permissions.set -principal " + testUser +
...
func setHostLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUser string, hosts []string) error {
	for i := 0; i < len(hosts); i++ {
		setPermissionForHosts := govcLoginCmd(vcAddress) + "govc permissions.set -principal " +
...
func setVMLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUserAlias string, testUser string, vms []string) error {
	for i := 0; i < len(vms); i++ {
		setPermissionForK8sVms := govcLoginCmd(vcAddress) + "govc permissions.set -principal " +
...
func setClusterLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUserAlias string, testUser string, cluster string) error {
	setPermissionForCluster := govcLoginCmd(vcAddress) + "govc permissions.set -principal " +
...
func setDataStoreLevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUserAlias string, testUser string, dataCenter string, datastores []string) error {
	for i := 0; i < len(datastores); i++ {
		setPermissionForDataStore := govcLoginCmd(vcAddress) + "govc permissions.set -principal " +
			testUserAlias + " " +
...
func setSearchlevelPermission(vcAddress string, sshClientConfig *ssh.ClientConfig,
	masterIp string, testUserAlias string, testUser string) error {
	setSearchLevelPermission := govcLoginCmd(vcAddress) + "govc permissions.set -principal " + testUserAlias +
