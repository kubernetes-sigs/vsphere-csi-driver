...
func InitCnsOperator(ctx context.Context, clusterFlavor cnstypes.CnsClusterFlavor,
	...
	if clusterFlavor == cnstypes.CnsClusterFlavorWorkload {
		...
		if commonco.ContainerOrchestratorUtility.IsFSSEnabled(ctx, common.SharedDiskFss) {
			// Create SharedBlockVolumeClients CRD from manifest if shared disk feature
			// is enabled.
			err = k8s.CreateCustomResourceDefinitionFromManifest(ctx,
				internalapiscnsoperatorconfig.EmbedCnsSharedBlockVolumeInfoFile,
				internalapiscnsoperatorconfig.EmbedCnsSharedBlockVolumeInfoFileName)
			if err != nil {
				log.Errorf("Failed to create %q CRD. Err: %+v", internalapis.CnsFileVolumeClientPlural, err)
				return err
			}
