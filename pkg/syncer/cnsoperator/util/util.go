...
func GetTKGVMIP(ctx context.Context, vmOperatorClient client.Client, dc dynamic.Interface,
	...
	if network_provider_type == NSXTNetworkProvider {
		...
		vpcs, found, err := unstructured.NestedSlice(networkInfoInstance.Object, "spec", "vpcs")
		if err != nil || !found || len(vpcs) == 0 {
			return "", fmt.Errorf("failed to get vpcs from networkinfo %s/%s with error: %v",
				vmNamespace, vmName, err)
		}

		vpc := vpcs[0].(map[string]string)
		ip, ok := vpc[vpcDefaultSnatIp]
		if !ok {
			return "", fmt.Errorf("defaultSNATIP not found in VPCState for networkinfo %s/%s",
				vmNamespace, vmName)
		}

		if ip == "" {
			return "", fmt.Errorf("spec.vpc[0].defaultSNATIP is not populated for "+
