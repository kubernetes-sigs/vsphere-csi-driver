...
func getVirtualMachine(ctx context.Context, vmName string, namespace string) (*vmoperatortypes.VirtualMachine, error) {
...
func getMaxWorkerThreadsToReconcileCnsFileAccessConfig(ctx context.Context) int {
	...
	if v := os.Getenv("WORKER_THREADS_FILE_ACCESS_CONFIG"); v != "" {
		...
		log.Debugf("WORKER_THREADS_REGISTER_VOLUME is not set. Picking the default value %d", defaultMaxWorkerThreadsForFileAccessConfig)
	}
