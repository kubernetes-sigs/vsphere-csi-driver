...
func TestReconcileDetachWithVolumeIDFallback(t *testing.T) {
	...
	// getVolumeID reads PVC/PV from the shared informer cache
	// (commonco.ContainerOrchestratorUtility), not from r.client — register the same
	// fixtures there too.
...
func TestReconcileDetachWithVolumeIDFallbackFailure(t *testing.T) {
	...
	instance := &v1a1.CnsNodeVmAttachment{
		...
		Spec: v1a1.CnsNodeVmAttachmentSpec{
			NodeUUID: "test-node-uuid",
			// "not-found-error" is FakeK8SOrchestrator's sentinel name for a PVC lookup
			// that returns apierrors.NewNotFound — see GetPvcObjectByName.
	...
	// getVolumeID reads PVC/PV from the shared informer cache
	// (commonco.ContainerOrchestratorUtility), not from r.client — wire up a fake with no
	// matching PVC so the "not-found-error" sentinel name resolves to NotFound.
	fakeCOIf, err := unittestcommon.GetFakeContainerOrchestratorInterface(common.Kubernetes)
	require.NoError(t, err)
	fakeCO, ok := fakeCOIf.(*unittestcommon.FakeK8SOrchestrator)
	require.True(t, ok)
...
func TestApplyAttachedPvcLabelToInstance(t *testing.T) {
	...
	// applyAttachedPvcLabelToInstance reads the PVC from the shared informer cache
	// (commonco.ContainerOrchestratorUtility), not from r.client — wire up a fake and let
	// each subtest register the PVCs it needs via SetPVCs.
	fakeCOIf, err := unittestcommon.GetFakeContainerOrchestratorInterface(common.Kubernetes)
	require.NoError(t, err)
	fakeCO, ok := fakeCOIf.(*unittestcommon.FakeK8SOrchestrator)
	require.True(t, ok)
	...
	t.Run("returns error when PVC fetch fails", func(t *testing.T) {
		...
		// "not-found-error" is FakeK8SOrchestrator's sentinel name for a PVC lookup that
		// returns apierrors.NewNotFound — see GetPvcObjectByName.
