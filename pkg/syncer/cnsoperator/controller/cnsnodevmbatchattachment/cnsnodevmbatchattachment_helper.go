...
func getVolumesToDetachForVmFromVC(ctx context.Context,
	...
	volumeIdsInSpec, volumeNamesInSpec, err := getPvcVolumeIdMapsInSpec(ctx, instance)
	if err != nil {
...
func validateBatchAttachRequest(ctx context.Context,
	...
	for _, accessMode := range pvc.Spec.AccessModes {
		...
		if accessMode == v1.ReadWriteMany || accessMode == v1.ReadOnlyMany {
			if batchAttachRequest.DiskMode == string(v1alpha1.Persistent) {
				return fmt.Errorf("incorrect input for PVC %s in namespace %s with accessMode %s. "+
					"DiskMode cannot be Persistent", pvcName, namespace, accessMode)
			}
		...
		if accessMode == v1.ReadWriteOnce {
			if batchAttachRequest.DiskMode == string(v1alpha1.IndependentPersistent) {
				return fmt.Errorf("incorrect input for PVC %s in namespace %s with accessMode %s. "+
					"DiskMode cannot be IndependentPersistent", pvcName, namespace, accessMode)
			}
			if batchAttachRequest.SharingMode == string(v1alpha1.SharingMultiWriter) {
				return fmt.Errorf("incorrect input for PVC %s in namespace %s with accessMode %s. "+
					"SharingMode cannot be Multiwriter", pvcName, namespace, accessMode)
			}
		}
	}
