...
// batchTrackingMockVolumeManager records CBT flag calls for setCBTFlagBeforeBatchAttach tests.
...
// TestSetCBTFlagBeforeBatchAttach_NoCBTConfig skips flag changes when
// IsCBTEnabledForNamespace returns an error (no CBTConfig CRs in the namespace).
func TestSetCBTFlagBeforeBatchAttach_NoCBTConfigSkipsFlagChange(t *testing.T) {
	ctx := context.Background()
	...
	r := &Reconciler{
		...
		cbtDynamicClient: newBatchCBTTestDynamicClient(t),
	}
	r.setCBTFlagBeforeBatchAttach(ctx, []string{"vol-1", "vol-2"}, "unknown-ns")

	assert.False(t, mockVM.setCalled)
	assert.False(t, mockVM.clearCalled)
}

func TestSetCBTFlagBeforeBatchAttach_CBTEnabled(t *testing.T) {
	ctx := context.Background()
	withCSIBackupAPIEnabled(t)
	...
	r.setCBTFlagBeforeBatchAttach(ctx, []string{"vol-a", "vol-b"}, "ns-cbt")

	assert.True(t, mockVM.setCalled)
	assert.False(t, mockVM.clearCalled)
}

func TestSetCBTFlagBeforeBatchAttach_CBTDisabled(t *testing.T) {
	ctx := context.Background()
	withCSIBackupAPIEnabled(t)
	...
	r.setCBTFlagBeforeBatchAttach(ctx, []string{"vol-x", "vol-y"}, "ns-off")

	assert.False(t, mockVM.setCalled)
	assert.True(t, mockVM.clearCalled)
}

func TestSetCBTFlagBeforeBatchAttach_DynamicClientNil(t *testing.T) {
	ctx := context.Background()
	...
	r.setCBTFlagBeforeBatchAttach(ctx, []string{"vol-1", "vol-2"}, "any-ns")
