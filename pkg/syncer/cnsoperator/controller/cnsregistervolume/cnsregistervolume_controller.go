...
func (r *ReconcileCnsRegisterVolume) Reconcile(ctx context.Context,
	...
	if pvc != nil {
		...
		if pvc.Spec.StorageClassName != nil && *pvc.Spec.StorageClassName != storageClassName {
			msg := fmt.Sprintf("Existing PVC %s has storage class %s, but storage policy %s maps to storage class %s",
				instance.Spec.PvcName, *pvc.Spec.StorageClassName, volume.StoragePolicyId, storageClassName)
			log.Error(msg)
			setInstanceError(ctx, r, instance, msg)
			// Untag the CNS volume which was created previously.
			...
			msg := fmt.Sprintf("Existing PVC %s has no storage class specified, but storage policy %s requires storage class %s",
				instance.Spec.PvcName, volume.StoragePolicyId, storageClassName)
			log.Error(msg)
			setInstanceError(ctx, r, instance, msg)
			// Untag the CNS volume which was created previously.
			_, delErr := common.DeleteVolumeUtil(ctx, r.volumeManager, volumeID, false)
