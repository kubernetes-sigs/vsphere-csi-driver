...
func (m *mockZonesProvider) RegisterZoneEventHandler(handler func(namespace string)) {
...
func TestMapZoneNamespaceToSPIs_EnqueuesAllInNamespace(t *testing.T) {
	...
	evtObj := &metav1.PartialObjectMetadata{
		ObjectMeta: metav1.ObjectMeta{Namespace: "ns1"},
	}
	reqs := r.mapZoneNamespaceToSPIs(ctx, evtObj)
	require.Len(t, reqs, 2)
	names := []string{reqs[0].Namespace + "/" + reqs[0].Name, reqs[1].Namespace + "/" + reqs[1].Name}
...
// returns nil for a nil object or an object without a namespace.
func TestMapZoneNamespaceToSPIs_NilAndEmpty(t *testing.T) {
	...
	assert.Nil(t, r.mapZoneNamespaceToSPIs(ctx,
		&metav1.PartialObjectMetadata{ObjectMeta: metav1.ObjectMeta{}}))
}

// TestEnqueueZoneNamespace_PushesEvent verifies that enqueueZoneNamespace pushes a
// GenericEvent stamped with the namespace onto the channel, skips empty
// namespaces, and drops events (without blocking) when the channel is full.
func TestEnqueueZoneNamespace_PushesEvent(t *testing.T) {
	...
	// A real namespace is pushed with the namespace stamped on the object.
	r.enqueueZoneNamespace("ns1")
	require.Len(t, r.zoneEventCh, 1)
	evt := <-r.zoneEventCh
	require.NotNil(t, evt.Object)
	assert.Equal(t, "ns1", evt.Object.GetNamespace())

	// Fill the buffer, then a further send must be dropped rather than block.
	r.enqueueZoneNamespace("ns2")
	require.Len(t, r.zoneEventCh, 1)
	r.enqueueZoneNamespace("ns3") // channel full -> dropped, must not block
...
func TestZoneAssignUnassign_TriggersTopologyUpdate(t *testing.T) {
	...
	zp.RegisterZoneEventHandler(r.enqueueZoneNamespace)
	require.NotNil(t, zp.zoneHandler, "handler should be registered")

	reconcileFromZoneEvent := func() {
		// Drain the event the handler pushed and run it through the mapper, then
		// reconcile each resulting request, mirroring the controller-runtime path.
		require.Len(t, r.zoneEventCh, 1)
		evt := <-r.zoneEventCh
		reqs := r.mapZoneNamespaceToSPIs(ctx, evt.Object)
		require.Len(t, reqs, 1)
