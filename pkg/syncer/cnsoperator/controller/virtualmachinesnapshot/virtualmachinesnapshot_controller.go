...
func (r *ReconcileVirtualMachineSnapshot) Reconcile(ctx context.Context,
	...
	err = r.reconcileNormal(ctx, vmSnapshot)
	if err != nil {
...
func (r *ReconcileVirtualMachineSnapshot) reconcileNormal(ctx context.Context,
	vmsnapshot *vmoperatorv1alpha4.VirtualMachineSnapshot) error {
	log := logger.GetLogger(ctx)
	// Check for the annotation "csi.vsphere.volume.sync: Requested"
	annotationValue := strings.ToLower(vmsnapshot.Annotations["csi.vsphere.volume.sync"])
	...
	if annotationValue == "requested" {
		...
		err = utils.UpdateVirtualMachineSnapshot(ctx, r.vmOperatorClient, vmsnapshot)
		if err != nil {
...
// validateStorageQuotaforVMSnapshot will fetch the volume-ids attached to virtualmachine
...
func (r *ReconcileVirtualMachineSnapshot) validateStorageQuotaforVMSnapshot(ctx context.Context, vm *vmoperatorv1alpha4.VirtualMachine) error {
	...
	log.Infof("Trigger CNS VolumeSync API, for no of volumes %d", len(syncVolumeSpecs))
	syncVolumeFaultType, err := r.volumeManager.SyncVolume(ctx, syncVolumeSpecs)
	if err != nil {
		log.Errorf("validateStorageQuotaforVMSnapshot: failed to sync volumes opertion "+
			"failed with cnsfault: %q error %+v", syncVolumeFaultType, err)
		return err
	}
