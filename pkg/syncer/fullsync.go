...
func RemoveCNSFinalizerFromPVCIfTKGClusterDeleted(ctx context.Context, k8sClient clientset.Interface,
	pvc *v1.PersistentVolumeClaim, finalizerToRemove string) {
	log := logger.GetLogger(ctx)
	...
	if len(pvc.ObjectMeta.Finalizers) != 0 {
		for i, finalizer := range pvc.ObjectMeta.Finalizers {
			...
			if err != nil && !apierrors.IsNotFound(err) {
				msg := fmt.Sprintf("RemoveFinalizerFromPVCIfTKGClusterDeleted: failed to get Cluster %q "+
				...
				log.Infof("RemoveFinalizerFromPVCIfTKGClusterDeleted: Removing %q finalizer from PVC "+
				...
				if err != nil {
					msg := fmt.Sprintf("RemoveFinalizerFromPVCIfTKGClusterDeleted: failed to update "+
...
func RemoveCNSFinalizerFromSnapIfTKGClusterDeleted(ctx context.Context, snapshotterClient versioned.Interface,
	vs *snapv1.VolumeSnapshot, finalizerToRemove string) {
	log := logger.GetLogger(ctx)
	...
	log.Infof("RemoveCNSFinalizerFromSnapIfTKGClusterDeleted called for vs %s/%s finalizer %d", vs.Namespace, vs.Name,
	...
	if len(vs.ObjectMeta.Finalizers) != 0 {
		for i, finalizer := range vs.ObjectMeta.Finalizers {
			...
			if err != nil && !apierrors.IsNotFound(err) {
				msg := fmt.Sprintf("RemoveFinalizerFromSnapIfTKGClusterDeleted: failed to get Cluster %q "+
				...
				// Remove finalizer if associated guest cluster is not-running/deleted already
				log.Infof("RemoveFinalizerFromSnapIfTKGClusterDeleted: Removing %q finalizer from VolumeSnapshot "+
					"with name: %q on namespace: %q in Terminating state",
				...
				if err != nil {
					msg := fmt.Sprintf("RemoveFinalizerFromSnapIfTKGClusterDeleted: failed to update "+
