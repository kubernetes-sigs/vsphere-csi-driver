...
func (rc *persistentVolumeClaimReconciler) delPVC(obj interface{}) {
	...
	err := runtime.DefaultUnstructuredConverter.FromUnstructured(obj.(*unstructured.Unstructured).Object,
		pvcObj)
	if err != nil {
		log.Errorf("delPVC: failed to cast object %+v to %s. Error: %v", obj, ResourceKindPVC, err)
		return
	}
...
func (rc *persistentVolumeClaimReconciler) syncPersistentVolumeClaimDelCase(ctx context.Context,
	...
	log := logger.GetLogger(ctx)
	log.Debugf("syncPersistentVolumeClaimDelCase: Started PersistentVolumeClaim processing %s/%s",
