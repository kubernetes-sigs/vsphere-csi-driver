...
func updateVolumeHealthStatus(ctx context.Context, k8sclient clientset.Interface,
	...
	if !found || val != volHealthStatus || !foundAnnHealthTS {
		...
		if err != nil {
			log.Errorf("updateVolumeHealthStatus: Failed to marshal updated PVC %s/%s with health annotations: %v", pvc.Namespace, pvc.Name, err)
			return
		}

		patchBytes, err := strategicpatch.CreateTwoWayMergePatch(oldData, newData, pvc)
		if err != nil {
			log.Errorf("updateVolumeHealthStatus: Error creating two way merge patch for PVC %s/%s with error: %v", pvc.Namespace, pvc.Name, err)
			return
		}
		...
		if err != nil {
			if apierrors.IsConflict(err) {
				...
				if err == nil {
					...
					if err != nil {
						log.Errorf("updateVolumeHealthStatus: Failed to marshal updated retry PVC %s/%s with health annotations: %v", newPvc.Namespace, newPvc.Name, err)
						return
					}

					retryPatchBytes, err := strategicpatch.CreateTwoWayMergePatch(oldRetryData, newRetryData, newPvc)
					if err != nil {
						log.Errorf("updateVolumeHealthStatus: Error creating two way merge patch for retry PVC %s/%s with error: %v", newPvc.Namespace, newPvc.Name, err)
						return
					}

					_, err = k8sclient.CoreV1().PersistentVolumeClaims(newPvc.Namespace).Patch(ctx, newPvc.Name, types.StrategicMergePatchType,
						retryPatchBytes, metav1.PatchOptions{})
					if err != nil {
