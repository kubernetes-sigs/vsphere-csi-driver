...
// TestCreateVACStoragePolicyUsageCRsForFullSync_SkipsWhenAPIUnavailable verifies
// that no SPUs are created when the VAC API is reported as unavailable.
// The vacAPIAvailableCheck function variable is replaced with a stub.
func TestCreateVACStoragePolicyUsageCRsForFullSync_SkipsWhenAPIUnavailable(t *testing.T) {
	const (
		ns       = "test-ns"
	...
	// Stub vacAPIAvailableCheck to return "not supported" without hitting a real cluster.
	origCheck := vacAPIAvailableCheck
	vacAPIAvailableCheck = func(_ context.Context) (bool, error) { return false, nil }
	defer func() { vacAPIAvailableCheck = origCheck }()

	ctx := context.Background()
	cnsClient := newVACFakeClient()

	// Provide a fake k8s client that has a matching VAC; it should never be listed.
	k8sFake := testclient.NewSimpleClientset()

	err := createVACStoragePolicyUsageCRsForFullSync(ctx, cnsClient, ns, k8sFake)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	spus := listSPUs(t, ctx, cnsClient, ns)
	if len(spus) != 0 {
		t.Errorf("expected 0 SPUs when VAC API is unavailable, got %d: %v", len(spus), spuNames(spus))
	}
}
