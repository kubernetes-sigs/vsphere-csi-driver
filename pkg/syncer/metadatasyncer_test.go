...
func TestSyncStorageQuotaReserved(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMetadataSyncer: func(t *testing.T) *metadataSyncInformer {
				// Create a fake k8s client and informer manager to get listers
				k8sClient := testclient.NewSimpleClientset()
				k8sInformerManager := k8s.NewInformer(ctx, k8sClient, true)
				// Start the informer to initialize listers
				stopCh := k8sInformerManager.Listen()
				if stopCh == nil {
					t.Fatal("Failed to start informer manager")
				}
				return &metadataSyncInformer{
					pvLister:  k8sInformerManager.GetPVLister(),
					pvcLister: k8sInformerManager.GetPVCLister(),
				}
			},
