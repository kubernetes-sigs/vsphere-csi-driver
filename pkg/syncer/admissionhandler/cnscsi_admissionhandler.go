...
func getPVCUID(ctx context.Context, pvcName, namespace string) (string, error) {
	...
	if err != nil {
		log.Errorf("failed to create k8s client. Err: %s", err)
		return "", err
	}
	...
	pvc, err := k8sClient.CoreV1().PersistentVolumeClaims(namespace).Get(ctx, pvcName,
		v1.GetOptions{})
	if err != nil {
		log.Errorf("failed to obtain PVC %s. Err: %s", pvcName, err)
		return "", err
	}
...
func (h *CSISupervisorMutationWebhook) mutateNewCnsFileAccessConfig(ctx context.Context,
	...
	if err != nil {
		log.Errorf("SALKO failed to get VM UID for VM %s", newCnsFileAccessConfig.Spec.VMName)
		return admission.Errored(http.StatusInternalServerError, err)
	}
