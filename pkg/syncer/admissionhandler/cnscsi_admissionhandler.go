...
func (h *CSISupervisorMutationWebhook) mutateNewCnsFileAccessConfig(ctx context.Context,
	...
	isCSIServiceAccount, err := validateCSIServiceAccount(req.UserInfo.Username)
	if err != nil {
		return admission.Errored(http.StatusInternalServerError, err)
	}

	if isCSIServiceAccount {
		log.Infof("Skip validation of CnsFileAccessConfig instance %s for CSI service account user",
