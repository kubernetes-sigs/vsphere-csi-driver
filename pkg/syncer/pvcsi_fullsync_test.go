...
func TestSyncGuestPvcCBTLabel_SkipsNonBlockVolumes(t *testing.T) {
	...
	// No interaction with supervisor or guest clients expected.
	for _, action := range guestClient.Actions() {
		assert.NotEqual(t, "patch", action.GetVerb())
	}
	assert.Empty(t, supervisorClient.Actions(), "supervisor client should not be queried for file volumes")
}
...
func TestSyncGuestPvcCBTLabel_SkipsUnboundPVs(t *testing.T) {
	...
	assert.Empty(t, supervisorClient.Actions(), "supervisor client should not be queried for unbound PVs")
}
