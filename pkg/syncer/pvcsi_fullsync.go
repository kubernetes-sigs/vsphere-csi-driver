		...
		if pv.Spec.ClaimRef != nil && pv.Status.Phase == v1.VolumeBound {
			...
			if metadataSyncer.coCommonInterface.IsFSSEnabled(ctx, common.SVPVCSnapshotProtectionFinalizer) {
				...
				if !cnsFinalizerPresent {
					...
					newFinalizerPVC.ObjectMeta.Finalizers = append(newFinalizerPVC.ObjectMeta.Finalizers, cnsoperatortypes.CNSVolumeFinalizer)
					...
					if err != nil {
						msg := fmt.Sprintf("error creating two way merge patch for supervisor PVC finalizer: %q in %q namespace. Error: %+v",
							pv.Spec.CSI.VolumeHandle, supervisorNamespace, err)
						log.Error(msg)
						continue
					}
