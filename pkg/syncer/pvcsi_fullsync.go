...
func PvcsiFullSync(ctx context.Context, metadataSyncer *metadataSyncInformer) error {
	...
	// Set csi.vsphere-volume labels and CNS finalizer on the Supervisor PVC which is
	// requested from TKC Cluster
	err = setGuestClusterDetailsOnSupervisorPVC(ctx, metadataSyncer, supervisorNamespace)
	if err != nil {
		log.Errorf("FullSync: Failed to set Guest Cluster labels or CNS finalizer on SupervisorPVC. Err: %v", err)
		return err
	}
	...
	if err != nil {
		log.Errorf("FullSync: Failed to set Guest Cluster labels or CNS finalizer on SupervisorSnapshot. Err: %v", err)
		return err
	}
...
func setGuestClusterDetailsOnSupervisorPVC(ctx context.Context, metadataSyncer *metadataSyncInformer,
	...
	for _, pv := range pvList {
		if pv.Spec.ClaimRef != nil && pv.Status.Phase == v1.VolumeBound {
			...
			// Add finalizer "cns.vmware.com/pvc-protection" to Supervisor PVC, if not present already,
			...
			cnsFinalizerPresent := slices.Contains(svPVC.ObjectMeta.Finalizers, cnsoperatortypes.CNSPvcFinalizer)
			if !cnsFinalizerPresent {
				svPVC.ObjectMeta.Finalizers = append(svPVC.ObjectMeta.Finalizers, cnsoperatortypes.CNSPvcFinalizer)
			}
...
func setGuestClusterDetailsOnSupervisorSnapshot(ctx context.Context, metadataSyncer *metadataSyncInformer,
	...
	for _, vsc := range vscList.Items {
		if (vsc.Spec.VolumeSnapshotRef.Name != "") &&
			...
			if err != nil {
				msg := fmt.Sprintf("setGuestClusterDetailsOnSupervisorSnapshot: failed to retrieve supervisor Snapshot %q "+
