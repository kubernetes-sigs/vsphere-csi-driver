...
func updatePVtoBackingDiskObjectIdMappingStatus(ctx context.Context, k8sclient clientset.Interface,
	...
	if !found || val != pvToBackingDiskObjectIdPair {
		...
		if err != nil {
			log.Errorf("updatePVtoBackingDiskObjectIdMappingStatus: Failed to marshal PVC %s/%s: %v", pvc.Namespace, pvc.Name, err)
			return false, err
		}
		...
		if err != nil {
			log.Errorf("updatePVtoBackingDiskObjectIdMappingStatus: Failed to marshal updated PVC %s/%s with annotation: %v", pvc.Namespace, pvc.Name, err)
			return false, err
		}

		patchBytes, err := strategicpatch.CreateTwoWayMergePatch(oldData, newData, pvc)
		if err != nil {
			log.Errorf("updatePVtoBackingDiskObjectIdMappingStatus: Error creating two way merge patch for PVC %s/%s: %v", pvc.Namespace, pvc.Name, err)
			return false, err
		}

		_, err = k8sclient.CoreV1().PersistentVolumeClaims(pvc.Namespace).Patch(ctx, pvc.Name, types.StrategicMergePatchType, patchBytes, metav1.PatchOptions{})
		if err != nil {
			if apierrors.IsConflict(err) {
				...
				if err != nil {
					log.Errorf("updatePVtoBackingDiskObjectIdMappingStatus: Failed to marshal retry PVC %s/%s: %v", newPvc.Namespace, newPvc.Name, err)
					return false, err
				}
				...
				if err != nil {
					log.Errorf("updatePVtoBackingDiskObjectIdMappingStatus: Failed to marshal updated retry PVC %s/%s with annotation: %v", newPvc.Namespace, newPvc.Name, err)
					return false, err
				}

				retryPatchBytes, err := strategicpatch.CreateTwoWayMergePatch(oldRetryData, newRetryData, newPvc)
				if err != nil {
					log.Errorf("updatePVtoBackingDiskObjectIdMappingStatus: Error creating two way merge patch for retry PVC %s/%s: %v", newPvc.Namespace, newPvc.Name, err)
					return false, err
				}

				_, err = k8sclient.CoreV1().PersistentVolumeClaims(newPvc.Namespace).Patch(ctx, newPvc.Name, types.StrategicMergePatchType, retryPatchBytes, metav1.PatchOptions{})
				if err != nil {
