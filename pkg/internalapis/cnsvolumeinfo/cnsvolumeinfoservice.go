...
// PatchVolumeInfo patches the CNSVolumeInfo instance associated with volumeID in given parameters.
func (volumeInfo *volumeInfo) PatchVolumeInfo(ctx context.Context, volumeID string, patchBytes []byte, allowedRetries int) error {
	log := logger.GetLogger(ctx)
	...
	for {
		attempt++
		log.Infof("attempt %d, to patch cnsvolumeinfo", attempt)
		...
		if err != nil && attempt >= allowedRetries {
			log.Infof("attempt: %d, failed to patch the cnsvolumeinfo", attempt)
			return err
		} else if err == nil {
			log.Infof("attempt: %d, Successfully patched the cnsvolumeinfo", attempt)
		}
