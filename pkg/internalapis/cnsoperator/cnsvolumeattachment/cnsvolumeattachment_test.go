...
func TestRemoveVmFromAttachedList(t *testing.T) {
	...
	t.Run("TestRemoveVmFromAttachedList", func(t *testing.T) {
		...
		err := cnsAttachmentInstance.RemoveVmFromAttachedList(context.Background(), volumeName, removeVmFromAttachedList)
		assert.NoError(t, err)
	...
	t.Run("TestRemoveVmFromAttachedListWhenLastVmIsRemaining", func(t *testing.T) {
		cnsAttachmentInstance := &cnsVolumeAttachment{
			client:     fakeClient,
			volumeLock: &sync.Map{},
		}
...
func TestCnsVolumeAttachmentExistsForPvc(t *testing.T) {
	testCnsvolumeAttachment := setupTestCnsNodeVmBatchAttachment()
	fakeClient := setTestEnvironment(testCnsvolumeAttachment)

	t.Run("TestCnsVolumeAttachmentExistsForPvc", func(t *testing.T) {
		cnsAttachmentInstance := &cnsVolumeAttachment{
			client:     fakeClient,
			volumeLock: &sync.Map{},
		}
		...
		volumeName := testNamespace + "/" + testCnsvolumeAttachmentName
		exists, err := cnsAttachmentInstance.CnsVolumeAttachmentExistsForPvc(context.Background(), volumeName)
		assert.NoError(t, err)

		assert.Equal(t, true, exists)
	})

	t.Run("TestCnsVolumeAttachmentExistsForPvcWhenInstanceIsNotThere", func(t *testing.T) {
		cnsAttachmentInstance := &cnsVolumeAttachment{
			client:     fakeClient,
			volumeLock: &sync.Map{},
		}

		volumeName := testNamespace + "/" + testCnsvolumeAttachmentName + "-fake"
		exists, err := cnsAttachmentInstance.CnsVolumeAttachmentExistsForPvc(context.Background(), volumeName)
		assert.NoError(t, err)
