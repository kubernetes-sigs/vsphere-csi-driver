...
func (c *K8sOrchestrator) PostLinkedCloneCreateAction(ctx context.Context, pvcName string, pvcNamespace string) error {
	...
	err = c.UpdateLinkedCloneSource(ctx, dataSource.Namespace, dataSource.Name, false)
	if err != nil {
...
func (c *K8sOrchestrator) GetLinkedCloneSource(ctx context.Context, pvcName string,
...
func (c *K8sOrchestrator) GetLinkedCloneSourceFromVolumeId(ctx context.Context, volumeId string,
	...
	if clusterFlavor == cnstypes.CnsClusterFlavorGuest {
		...
		pvName = "pvc-" + pvcUUID
		log.Infof("GetLinkedCloneSourceFromVolumeId: retrieved PV %s from volumeId %s in Guest Cluster",
			pvName, volumeId)
		...
		log.Infof("GetLinkedCloneSourceFromVolumeId: retrieved PV %s from volumeId %s in Supervisor Cluster",
			pvName, volumeId)
	} else {
	...
	sourceNamespaceName := persistentVolume.Spec.CSI.VolumeAttributes[common.VolumeContextAttributeLinkedCloneSource]
...
// created from it by incrementing the ref count. For LinkedClone deletion the ref count is decremented.
func (c *K8sOrchestrator) UpdateLinkedCloneSource(ctx context.Context, sourceNamespace, sourceName string,
	isDelete bool) error {
	log := logger.GetLogger(ctx)
	var err error
	if !isDelete {
