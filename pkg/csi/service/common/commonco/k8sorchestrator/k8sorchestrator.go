...
var (
	...
	// wcpCapabilitiesMutex is the mutual exclusion lock used for accessing global map WcpCapabilitiesMap.
	wcpCapabilitiesMutex = &sync.RWMutex{}
)
...
func SetWcpCapabilitiesMap(ctx context.Context, cnsOperatorClient client.Client) error {
	log := logger.GetLogger(ctx)
	wcpCapabilitiesMutex.Lock()
	defer wcpCapabilitiesMutex.Unlock()
...
// setCapabilitiesMapOnSupervisor creates a new cns-operator client for supervisor cluster and
// calls SetWcpCapabilitiesMap function to set latest capabilities values in global wcp capabilities map.
func setCapabilitiesMapOnSupervisor(ctx context.Context) error {
	log := logger.GetLogger(ctx)
	...
	err = SetWcpCapabilitiesMap(ctx, cnsOperatorClient)
	if err != nil {
	...
	if c.clusterFlavor == cnstypes.CnsClusterFlavorVanilla {
		...
		if _, exists := common.WCPFeatureStates[featureName]; exists {
			...
			if len(WcpCapabilitiesMap) == 0 {
				err = setCapabilitiesMapOnSupervisor(ctx)
				if err != nil {
					log.Errorf("failed to set WCP capabilities map, Err: %+v", err)
					return false
				}
