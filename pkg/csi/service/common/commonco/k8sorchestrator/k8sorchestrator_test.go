...
func TestInitPVCToSnapshotsMap(t *testing.T) {
	t.Run("SkipForNonGuestCluster", func(tt *testing.T) {
		ctx := context.Background()

		for _, flavor := range []cnstypes.CnsClusterFlavor{
			cnstypes.CnsClusterFlavorWorkload,
			cnstypes.CnsClusterFlavorVanilla,
		} {
			err := initPVCToSnapshotsMap(ctx, flavor)
			assert.NoError(tt, err, "expected no error for cluster flavor %s", flavor)
		}
	})
