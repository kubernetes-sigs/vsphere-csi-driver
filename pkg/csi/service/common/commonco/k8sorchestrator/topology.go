...
// StartZonesInformer), the event handler is attached immediately; otherwise it
// is a no-op on the informer side until StartZonesInformer is called.
func (c *K8sOrchestrator) RegisterZoneEventHandler(handler func(namespace string)) {
	if handler == nil {
		return
	}
	...
	if informer != nil {
		if _, err := informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
			...
			// and will fire if StartZonesInformer is called later.
...
// GetAccessibleZonesForDatastore returns all zones where the specified datastore is accessible.
// This is a placeholder implementation for vanilla clusters.
func (volTopology *controllerVolumeTopology) GetAccessibleZonesForDatastore(ctx context.Context,
	...
	// how zones are configured in vanilla environments.
