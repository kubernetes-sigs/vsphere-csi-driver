...
// TestCreateFileVolumeViaFVS_StandaloneSupervisorWithoutFileStoreLabel verifies that a Ready FileVolume
// with an export path and an endpoint, but no fvs.vcf.broadcom.com/file-store label, is still treated
// as fully provisioned. A standalone supervisor not backed by VCFA never sets this label, so it must
// be optional: CreateVolume succeeds and the response volume context simply omits the key, rather than
// the driver waiting for a label that will never arrive.
func TestCreateFileVolumeViaFVS_StandaloneSupervisorWithoutFileStoreLabel(t *testing.T) {
	pvcUID := "cccccccc-cccc-cccc-cccc-cccccccccccc"
	cases := []struct {
		name   string
		labels map[string]string
		...
		{name: "file store label present but empty", labels: map[string]string{common.FileStoreLabelKey: ""}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ctx := context.Background()
			...
			require.Equal(t, common.FVSVolumeIDPrefix+"inst-ns:"+pvcUID, resp.Volume.VolumeId)
			require.Equal(t, "10.0.0.10:/vsanfs/fs-1", resp.Volume.VolumeContext[common.Nfsv4ExportPathAnnotationKey])
			_, ok := resp.Volume.VolumeContext[common.FileStoreLabelKey]
			require.False(t, ok, "file store key must be omitted, not merely empty, when the label is absent")
		})
	}
}

// TestCreateFileVolumeViaFVS_IncompleteReadyTimesOut verifies that a Ready FileVolume missing the
// export path or the endpoint is not treated as provisioned: the poll keeps waiting and CreateVolume
// ultimately fails with DeadlineExceeded. Unlike the file-store label, these two fields are always
// required.
func TestCreateFileVolumeViaFVS_IncompleteReadyTimesOut(t *testing.T) {
	...
	existing := &fvv1alpha1.FileVolume{
		...
		Status: fvv1alpha1.FileVolumeStatus{
			Phase:      fvv1alpha1.FileVolumePhaseReady,
			ExportPath: "/vsanfs/fs-1",
			// Endpoint deliberately left unset.
		},
	}
	c, req := newFileStoreTestController(t, pvcUID, existing)
	...
	require.Contains(t, err.Error(), "export path and endpoint")
}
