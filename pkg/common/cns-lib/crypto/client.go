...
func (c *defaultClient) MarkEncryptedVAC(
	...
	switch {
	case encrypted && vacIsOwner:
		// The VAC should be marked encrypted, which means it should be in the
		// ConfigMap. Since the VAC is currently set in the ConfigMap, we can
		// return early.
		return nil
	case !encrypted && !vacIsOwner:
		// The VAC should be marked as not encrypted, which means it should not be
		// in the ConfigMap. Since the VAC is not currently in the ConfigMap, we
		// can return early.
		return nil
	}
