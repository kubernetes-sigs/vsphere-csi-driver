...
func (m *mockVirtualCenter) QueryAllProfileDetails(ctx context.Context, profileCategory string, fetchAllFields bool) ([]ProfileDetail, error) {
	args := m.Called(ctx, profileCategory, fetchAllFields)
	return args.Get(0).([]ProfileDetail), args.Error(1)
}
