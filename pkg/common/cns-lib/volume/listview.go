...
func (l *ListViewImpl) ResetVirtualCenter(ctx context.Context, virtualCenter *cnsvsphere.VirtualCenter) {
	log := logger.GetLogger(ctx)
	log.Info("trying to update VirtualCenter object reference in ListView")
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *ListViewImpl) isConnected() error {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *ListViewImpl) listenToTaskUpdates() {
	...
	for {
		...
		log.Infof("attempting lock before ")
		l.mu.Lock()
		if recreateView {
