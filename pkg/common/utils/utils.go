...
func ListVirtualMachines(ctx context.Context, clt client.Client,
	namespace string) (*vmoperatorv1alpha2.VirtualMachineList, error) {
	log := logger.GetLogger(ctx)
	vmList := &vmoperatorv1alpha2.VirtualMachineList{}
...
func GetVirtualMachine(ctx context.Context, vmKey types.NamespacedName,
	vmOperatorClient client.Client) (*vmoperatorv1alpha2.VirtualMachine, string, error) {
	log := logger.GetLogger(ctx)
...
func PatchVirtualMachine(ctx context.Context, vmOperatorClient client.Client,
	vm, old_vm *vmoperatorv1alpha2.VirtualMachine) error {
	log := logger.GetLogger(ctx)
...
func GetVirtualMachineListAllApiVersions(ctx context.Context, namespace string,
	vmOperatorClient client.Client) (*vmoperatorv1alpha2.VirtualMachineList, error) {
	log := logger.GetLogger(ctx)
	...
	if namespace != "" {
		...
		if err != nil {
			log.Errorf("error while fetch virtualmachines in namespace %s", namespace)
			return nil, err
		}
	} else {
		...
		if err != nil {
			log.Error("error while fetch list of all virtualmachines")
			return nil, err
		}
	}
