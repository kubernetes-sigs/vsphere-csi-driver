...
func ListVirtualMachines(ctx context.Context, vmOperatorClient client.Client,
	namespace string) (*vmoperatortypes.VirtualMachineList, error) {
...
// ListVirtualMachinesAllNamespaces lists all the virtual machines from all namespace
func ListVirtualMachinesAllNamespaces(ctx context.Context,
	...
	log.Infof("Successfully listed %d virtual machines in all namespaces",
		len(vmList.Items))
	return vmList, nil
}
...
func GetVirtualMachine(ctx context.Context, vmKey types.NamespacedName,
	vmOperatorClient client.Client) (*vmoperatortypes.VirtualMachine, string, error) {
	log := logger.GetLogger(ctx)
	vm := &vmoperatortypes.VirtualMachine{}
	log.Infof("get machine with vm-operator api version v1alpha2 name: %s, namespace: %s",
		vmKey.Name, vmKey.Namespace)
	apiVersion := vmOperatorApiVersionPrefix + "/v1alpha2"
	err := vmOperatorClient.Get(ctx, vmKey, vm)
	if err != nil {
		log.Errorf("GetVirtualMachine: failed to get VirtualMachine "+
...
func PatchVirtualMachine(ctx context.Context, vmOperatorClient client.Client,
	vm, old_vm *vmoperatortypes.VirtualMachine) error {
	log := logger.GetLogger(ctx)
